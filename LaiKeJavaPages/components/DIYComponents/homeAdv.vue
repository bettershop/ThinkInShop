<template>
    <div :style="{background: bgColor2[0].item}">
        <view class="homeAdv"
              :style="{background: `linear-gradient(90deg,${bgColor[0].item} 0%,${bgColor[1].item} 100%)`, marginTop: `${pxToRpxNum(marginTop)}rpx`}"
            ...
            <image v-for="(item, index) of imgUrls" :src="LaiKeTuiCommon.getFastImg(item.img == '' ? default_img : item.img, 664, 320)" lazy-load="true" @error="errorImg(index)" :class="[imgConfig?'':'fillet']" @click="$emit('toUrl', item.info[1].value)"></image>
        </view>    
    </div>
...
<script>
    export default {
        ...
        methods: {
            errorImg(index) {
               this.imgUrls[index].img="../../static/img/default-img.png"
            },
        }
    }
</script>
