                ...
                <view v-if="!opensetting">
                    <scroll-view scroll-x="true" class="scroll_view_gundong">
                        ...
                        >
                            <view>
                                <image
                                    :src="LaiKeTuiCommon.getFastImg(item.backImgUrl == '' ? default_img : item.backImgUrl, 480, 300)"
                                    mode="aspectFill"
                                    lazy-load="true"
                                    @error="backErrorImg(index)"
                                ></image>
                            </view>
                            <view>
                                <view>
                                    <image
                                        :src="LaiKeTuiCommon.getFastImg(item.headimgurl  == '' ? Default_picture : item.headimgurl, 120, 120)"
                                        mode="aspectFill"
                                        lazy-load="true"
                                        @error="headErrorImg(index)"
                                    ></image>
                                </view>
                            </view>
                            <view>{{ item.name }}</view>
...
export default {
    ...
    data() {
        return {
            ...
            Default_picture:"../../static/img/Default_picture.png"
        };
    },
    created(){
        this.axios()
    ...
    methods: {
        headErrorImg(index) {
            this.isData[index].headimgurl="../../static/img/Default_picture.png"
        },
        backErrorImg(index) {
            this.isData[index].backImgUrl="../../static/img/default-img.png"
        },
        axios(limitNum = 3) {
        	let data = { 
        	...
        	this.$req.post({
                ...
                try{ 
                    if (res.code == 200) {
                        this.isData = res.data.r_mch
                        console.log('woshidp', this.isData)
                    } else {
                        uni.showToast({
                            title: res.message,
                            icon: 'none'
