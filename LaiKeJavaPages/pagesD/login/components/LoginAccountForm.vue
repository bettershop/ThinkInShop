<template>
  <div class="login-form">
    <div class="uni-input">
      <image :src="xzh" mode="aspectFill"></image>
      <input
        class="input" 
        v-model="localAccount"
        :placeholder="language.newlogin.srzh"
        @input="inputAccount"
      />
      <image v-if="localAccount != ''" :src="xgb" @click="clearFrom" mode="aspectFill"></image>
    </div>
    <div class="uni-input">
...
export default {
  ...
  methods: {
    inputAccount() {
      // 触发登录逻辑
      this.$emit('inputAccount', this.localAccount);
    },
    inputPassword() {
      // 触发登录逻辑
