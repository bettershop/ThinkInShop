<template>
    <view class="isSearch" :style="{top:is_search_top}">
        <!-- 有搜索按钮的 搜索框 -->
        <template v-if="is_search_type == 1">
            <view class="is_search"
                :style="{
                ...
                <view>
                    <view class='search_input'>
                        ...
                        <input v-model="is_search" type="text" :placeholder="is_search_prompt" />
                        <image v-show="is_search.length > 0" @tap="_clearData" class="cancel" :src="sc_icon" mode=""></image>
                    </view>
                    <view class='search_btn' @tap="_isSearch"
                    :style="{
                        backgroundColor: is_search_btn_bgColor,
                        color: is_search_btn_color
                    }">
                        {{language.aComponents.search.ss}}
                    </view>
                </view>
            </view>
        </template>
            ...
            <view class="is_search"
                ...
                <view>
                    <view class='search_input no_btn'>
                        <image class="searchImg" :src="serchimg" alt=""></image>
                        <input @confirm="_isSearch" v-model="is_search" type="text" :placeholder="is_search_prompt" />
                        <image v-show="is_search.length > 0" @tap="_clearData" class="cancel" :src="sc_icon" mode=""></image>
                    </view>
                </view>
            </view>
...
<script>
    export default{
        props:{
            ...
            //必传  否则有遮挡   在父组件中计算top传入：缓存中的data_height + heads组件高度 + 其他组件高度（如果有）
            "is_search_top":{
                type: String,
                default: '0' //定位 上边距
            },
