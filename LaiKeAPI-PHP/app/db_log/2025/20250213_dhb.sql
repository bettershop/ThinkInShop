...
CHANGE COLUMN `G_Delete` `district_delete` int(11) NULL DEFAULT 0 COMMENT '是否删除（未用）' AFTER `district_childcount`,
CHANGE COLUMN `G_Num` `district_num` int(11) NULL DEFAULT 0 COMMENT '（未用）' AFTER `district_delete`,
ADD COLUMN `district_country_num` int(11) NULL COMMENT '国家编码' AFTER `district_num`,
DROP PRIMARY KEY,
