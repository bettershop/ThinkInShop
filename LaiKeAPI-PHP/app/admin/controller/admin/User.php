...
{   
    private function saveUserImportRecord($store_id, $mch_id, $fileName, $status, $reason, $time)
    {
        $logData = array(
            'store_id' => $store_id,
            'name' => $fileName,
        ...
        return Db::name('file_delivery')->insert($logData);
    }
    ...
    public function uploadAddUser()
    {
        ...
        {   
            ...
            else
            {
                ...
                $res_log = $this->saveUserImportRecord($store_id, $mch_id, $name, 0, $reason, $time);
                if($res_log < 1)
                {
        ...
        fclose($handle);//关闭指针
        //保存记录
        $res_log = $this->saveUserImportRecord($store_id, $mch_id, $name, 1, '', $time);
        if($res_log < 1)
        {
            Db::rollback();
    ...
    public function uploadRecordList()
    {
        ...
        $condition = " store_id = '$store_id' and type = 2 ";
        if($key != '')
        {
            $key = Tools::FuzzyQueryConcatenation($key);
        ...
        if($total > 0)
        {
            $sql = "select id,name,status,text,add_date,mch_id from lkt_file_delivery where $condition order by add_date desc limit $start,$pagesize";
            $res = Db::query($sql);
            if($res)
            {
                $list = $res;
    ...
    public function delUploadRecord()
    {
        ...
        $res = Db::name('file_delivery')->where('id',$id)->where('store_id',$store_id)->where('type',2)->delete();
        if($res < 1)
        {
