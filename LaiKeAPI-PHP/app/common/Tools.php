...
class Tools
{
    ...
    public function generate_code($array)
    {
        ...
        if (empty($mobile))
        {
            return $this->formatSmsResponse(['code' => ERROR_CODE_DXMBBCZ, 'message' => Lang('tools.1'), 'data' => false], $type);
        }

        Db::execute("delete from lkt_session_id where date_add(add_date, interval 5 minute) < now()");
        ...
        if (empty($r0))
        {
            return $this->formatSmsResponse(['code' => ERROR_CODE_DXFSSB, 'message' => Lang('tools.1'), 'data' => false], $type);
        }
        ...
        {
            $sql2 = "select SignName,TemplateCode,content from lkt_message where store_id = '$store_id' and international = '$international' and type = '$type' and type1 = '$type1'";
            $r2 = Db::query($sql2);
            ...
            $content = array_combine($result[0], array_fill(0, count($result[0]), ''));
            ...
            if ($content)
            {
                foreach ($content as $k => $v)
                {
                    ...
                    elseif ($k == 'orderno')
                    {
                        $content['orderno'] = $bizparams['sNo'] ?? $bizparams['orderno'] ?? '';
                    }
                    elseif ($k == 'store')
                    {
        ...
        $content1 = json_encode([$mobile, $TemplateParam]);
        ...
        if ($type == 0)
        {
            if ($res->Code == 'OK')
            {
                ...
                if ($r1)
                {
                    foreach ($r1 as $k => $v)
                    {
                        $content2 = json_decode($v['content'], true);
                        if ($mobile == ($content2[0] ?? ''))
                        {
                            Db::name('session_id')->where('id', $v['id'])->update(['content' => $content1]);
                            $rew = 1;
                        }
                    }
                }
                if ($rew == 0)
                {
                    Db::name('session_id')->insert(['content' => $content1, 'add_date' => $time]);
                }

                return $this->formatSmsResponse(['code' => '200', 'message' => Lang('Success'), 'data' => true], $type);
            }

            $message = $this->getSmsErrorMessage($res->Code);
            return $this->formatSmsResponse(['code' => ERROR_CODE_DXFSSB, 'message' => $message, 'data' => false], $type);
        }

        return $this->formatSmsResponse($res, $type);
    }

    private function getSmsErrorMessage($code)
    {
        $errorMap = [
            'isv.OUT_OF_SERVICE' => 'tools.2',
            'isv.SMS_TEMPLATE_ILLEGAL' => 'tools.3',
            'isv.SMS_SIGNATURE_ILLEGAL' => 'tools.4',
            'isv.MOBILE_NUMBER_ILLEGAL' => 'tools.6',
            'isv.MOBILE_COUNT_OVER_LIMIT' => 'tools.5',
            'isv.BUSINESS_LIMIT_CONTROL' => 'tools.7',
            'isv.AMOUNT_NOT_ENOUGH' => 'tools.8',
        ];

        $key = $errorMap[$code] ?? 'tools.9';
        return Lang($key);
    }

    private function formatSmsResponse($payload, $type)
    {
        $json = json_encode($payload);
        if ((int)$type === 0)
        {
            // 历史兼容：验证码短信接口依赖 generate_code 直接输出并终止。
            echo $json;
            exit;
        }
