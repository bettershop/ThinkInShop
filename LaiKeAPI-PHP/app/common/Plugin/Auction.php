...
class Auction 
{
    ...
    public function OpenSpecial($store_id)
    {
        ...
        // 根据专场开始时间，查询未开始专场 
        $r0 = AuctionSpecialModel::where(['store_id'=>$store_id,'recovery'=>0,'status'=>0])->where('start_date','<=',$time)->field('id')->select()->toArray();
        if($r0)
        {
            foreach($r0 as $k0 => $v0)
            {
                $id0 = $v0['id']; // 专场ID
                // 修改专场为进行中
                $sql0_1 = array('id'=>$id0,'recovery'=>0,'status'=>0);
                $r0_1 = Db::name('auction_special')->where($sql0_1)->update(['status' => 1]);
                if($r0_1 < 1)
                {
                ...
                // 修改场次为进行中
                $sql0_2 = array('special_id'=>$id0,'recovery'=>0,'status'=>0);
                $r0_2 = Db::name('auction_session')->where($sql0_2)->where('start_date','<=',$time)->update(['status' => 1]);
                if($r0_2 < 1)
                {
    ...
    public function GetPromiseList($array)
    {
        ...
        //初始化分页查询条件
        $condition = " a.store_id = '$store_id' and a.special_id = '$specialId' and a.recovery = 0 and b.store_id = '$store_id' ";
        if ($key != '')
        {
            $key_0 = Tools::FuzzyQueryConcatenation($key);
