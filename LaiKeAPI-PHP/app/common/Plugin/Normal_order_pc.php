    ...
    public function payment(&...$context)
    {
        ...
        {   
            ...
            foreach ($v['list'] as $key => $value)
            {
                ...
                $sql_insert = array('store_id'=>$store_id,'user_id'=>$user_id,'p_id'=>$pid,'p_name'=>$product_title,'p_price'=>$value['price'],'num'=>$num,'unit'=>$value['unit'],'r_sNo'=>$sNo,'add_time'=>date("Y-m-d H:i:s"),'r_status'=>$order_status,'size'=>$value['size'],'sid'=>$cid,'freight'=>$freight_price,'coupon_id'=>$value['coupon_id'],'after_discount'=>$value['amount_after_discount'],'supplier_settlement'=>$value['supplier_settlement']);
