...
class simple_html_dom_node
{
    ...
    public function find($selector, $idx = null, $lowercase = false)
    {
        ...
        if (is_null($idx)) {return $found;} elseif ($idx < 0) {$idx = count($found) + $idx;}
    ...
    public static function is_utf8($str)
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            $c = ord($str[$i]);
            if ($c > 128) {
                if (($c >= 254)) {return false;} elseif ($c >= 252) {$bits = 6;} elseif ($c >= 248) {$bits = 5;} elseif ($c >= 240) {$bits = 4;} elseif ($c >= 224) {$bits = 3;} elseif ($c >= 192) {$bits = 2;} else {return false;}
