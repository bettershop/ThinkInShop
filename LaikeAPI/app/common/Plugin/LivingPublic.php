...
class LivingPublic 
{
    ...
    public function payment(&...$context)
    {
        ...
        //查询默认地址order_details
        $address = Tools::find_address($store_id, $user_id,$no_delivery_str, $address_id);
        $cpc = $fullPhone?$fullPhone:$address['cpc'];
        $shop_cpc = $cpc;
        ...
        {   
            ...
            foreach ($v['list'] as $key => $value)
            {
                ...
                $sql_living = "select user_id from lkt_living_room where id = '$living_id' ";
                ...
                $sql_stock = "update lkt_living_product set num = num - '$num' where store_id = '$store_id' and living_id = '$living_id' and pro_id = '$pid' and config_id = '$cid' ";
                $res_stock = Db::execute($sql_stock);
                if($res_stock < 1)
                {
                    Db::rollback();
                    $message = Lang('operation failed');
                    echo json_encode(array('code' => 109, 'message' => $message, 'data'=>array('line' => __LINE__)));
                    exit;
                }
    ...
    {
        $is_status = 0;
        $is_status = Db::name('living_config')->where(['store_id'=>$store_id])->value('is_open as status');
      
        return $is_status;
    }

    // 退还库存
    public static function Return_inventory($array)
    {
        $store_id = $array['store_id']; // 商城ID
        $user_id = $array['user_id']; // 购买人user_id
        $living_id = $array['living_id']; // 直播间ID
        $sNo = $array['sNo']; // 订单号
        $pro_id = $array['pro_id']; // 商品ID
        $config_id = $array['config_id']; // 属性ID
        $num = $array['num']; // 数量
        $source = $array['source']; // 1.取消订单 2.售后
        
        $sql0 = "select living_status,user_id from lkt_living_room where store_id = '$store_id' and id = '$living_id' ";
        $r0 = Db::query($sql0);
        if($r0)
        {
            $living_status = $r0[0]['living_status'];
            $anchor_id = $r0[0]['user_id'];

            if($living_status == 1)
            { // 直播中(库存退还到主播商品)
                if($source == 1)
                {
                    $sql1 = "update lkt_living_product set num = num + '$num' where living_id = '$living_id' and pro_id = '$pro_id' and config_id = '$config_id' and recycle = 0 ";
                }
                else
                {
                    $sql1 = "update lkt_living_product set num = num + '$num',xl_num = xl_num - '$num' where living_id = '$living_id' and pro_id = '$pro_id' and config_id = '$config_id' and recycle = 0 ";
                }
                $r1 = Db::execute($sql1);
            }
            else
            { // 直播结束（库存退还到商品）
                if($source == 1)
                {
                    $array = array('store_id'=>$store_id,'supplier_id'=>0,'pid'=>$pro_id,'sid'=>$config_id,'num'=>$num,'type'=>3,'user_id'=>$user_id);
                }
                else
                {
                    $array = array('store_id'=>$store_id,'supplier_id'=>0,'pid'=>$pro_id,'sid'=>$config_id,'num'=>$num,'type'=>2,'user_id'=>$user_id);
                }
                $res_stock = PC_Tools::Modify_inventory($array);
            }

            $sql3 = "select commission from lkt_living_commission where store_id = '$store_id' and s_no = '$sNo' and recycle = 0 and status = 101";
            $r3 = Db::query($sql3);
            if($r3)
            {
    ...
    {
        $lktlog = new LaiKeLogUtils();
        $lktlog->log("app/LivingPublic.log",$Log_content);
        return;
    }
}
