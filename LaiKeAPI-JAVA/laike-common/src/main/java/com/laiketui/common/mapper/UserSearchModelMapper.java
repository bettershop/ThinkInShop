...
public interface UserSearchModelMapper extends BaseMapper<UserSearchModel>
{
     ...
     */
    @Select("select keyword from lkt_user_search where store_id = #{storeId} and token = #{token} and add_date >= #{startDate} and add_date <= #{endDate} and user_id = '' order by add_date desc")
     ...
     */
    @Update("<script>" +
            " update lkt_user_search set add_date = #{sysTime} where store_id = #{storeId} " +
            " <if test='userId != null '> and user_id = #{userId} </if> " +
            " <if test='token != null '> and token = #{token} </if> " +
            "</script>")
    int updateAddDate(Date sysTime, int storeId, String userId, String token, String key);
