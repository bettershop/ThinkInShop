...
public interface GuideModelMapper extends BaseMapper<GuideModel>
{
    ...
    @Update("update lkt_guide_menu set guide_sort = #{sort} where  role_id = #{roleId} and menu_id = #{mid}")
    ...
    @Update("update lkt_guide_menu a,lkt_guide_menu b set a.guide_sort= b.guide_sort,b.guide_sort=a.guide_sort " +
            " where  a.role_id=#{roleId} and b.role_id=#{roleId} and a.menu_id=#{id} and b.menu_id=#{moveId}")
     ...
     */
    @Update("update lkt_guide_menu set is_display = #{show} where role_id = #{roleId} and menu_id = #{mid}")
    int displaySwitch(int roleId, int mid, int show);
}