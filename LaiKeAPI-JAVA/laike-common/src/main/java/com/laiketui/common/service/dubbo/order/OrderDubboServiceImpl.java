...
import com.laiketui.common.utils.Arith;
 ...
 * @author wangxian
 */
@HandlerOrderType(type = DictionaryConst.OrdersType.ORDERS_HEADER_GM)
@Service
public class OrderDubboServiceImpl implements OrderDubboService
{
    ...
    public Map<String, Object> settlement(OrderVo vo) throws LaiKeAPIException
    {
        try
        {
            ...
            //pid商品id，cid商品规格id，num数量，sid秒杀id
            //  cart_id: 3010,3009
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_CSGSBZQ, vo.getProductsInfo() + ">参数格式不正确", "immediatelyCart");
            }
            //主要根据商品类型的不同来判断商品是否存在或者库存是否充足，包括购物车中商品Example的处理，productsListMap中保存着商品、规格、库存
            //单个商品为productList传入，以购物车传入则为vo.getCarts()，BuyType()支付方式，
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_KCBZ, "库存不足2", "settlement");
            }
            //订单中有供应商商品则不支持自提
            //不计算最优优惠卷
            boolean isZiti = true;
            //productsListMap中pid，num，cid
            ...
            else if (vo.getIs_self_delivery() != null && vo.getIs_self_delivery() == 2)
            {
                for (Map<String, Object> mchProductsInfo : products)
                {
                    //商家自配免运费
                    ...
                    BigDecimal freightPrice = DataUtils.getBigDecimalVal(mchProductsInfo, "freight_price");
                    mchProductsInfo.put("product_total", productTotal.subtract(freightPrice));
                    List<Map<String, Object>> productInfoList = (List<Map<String, Object>>) mchProductsInfo.get("list");
                    //商家自配不计算运费
                    for (Map<String, Object> infoMap : productInfoList)
                    {
                        infoMap.put("freight_price", BigDecimal.ZERO);
                    }
                }
            }
            else
            ...
            if (products.size() == 1)
            {
                int is_self_delivery = 1;
                for (Map<String, Object> mchProductsInfo : products)
                {
                    Integer shop_id = DataUtils.getIntegerVal(mchProductsInfo, "shop_id");
                    if (shop_id != null)
                    {
                        MchModel mchModel = mchModelMapper.selectByPrimaryKey(shop_id);
                        is_self_delivery = mchModel.getIs_self_delivery() * is_self_delivery;
                        if (is_self_delivery > 0)
                        {
                            zpStatus = 1;
                        }
                    }
                    //供应商商品处理
                    List<Map<String, Object>> proList = (List<Map<String, Object>>) mchProductsInfo.get("list");
                    for (Map<String, Object> proInfo : proList)
                    {
                        if (proInfo.containsKey("gongyingshang") && StringUtils.isNotEmpty(DataUtils.getStringVal(proInfo, "gongyingshang")))
                        {
                            isGysGood = 1;
                            zpStatus = 0;
                        }
            ...
            //0,356,coupon，前端传来的基本形式，当没有优惠券类型时自动添加一个没使用优惠券
            if (platformDiscountIdList.length == 2)
            {
                //给优惠券列表再加一个标志位，且加一位["325", "326", null]
            ...
            // 最后一调数据的键名，其意义为使用的是何种的优惠券coupon，substration，nodiscount，记录最后一个优惠券的下标，当使用两张优惠券的时候，则必为无折扣
            int platformDiscountTypePos = platformDiscountIdList.length - 1;
            // 平台优惠的键名，当使用两张优惠券的时候，为第二张优惠券名
            ...
            //使用的优惠券的类型，对应着平台优惠券或者是店铺优惠券coupon，满减券substration，无折扣nodiscount
            String discountType = platformDiscountIdList[platformDiscountTypePos];
            //String coupon:platformDiscountIdList
            //platformDiscountIdList中当没使用优惠券时则直接跳过该for循环
            //遍历优惠券集合
            ...
            // 获取店铺的优惠券 是否是商品自配，此参数0为自提，1为快递，2为商家自配
            Integer is_self_driver = vo.getIs_self_delivery();
            if (is_self_driver == null)
            {
                is_self_driver = 1; //按前端逻辑来 0为自提，1为快递，2为商家自配
            }
...
/*            //禅道48455 可以支付0元
            ...
            boolean scoreDeductionValue = false;
            if (vo.getType().equals(DictionaryConst.OrdersType.ORDERS_HEADER_GM))
            {
                if (user.getScore() > 0)
                {
                    BigDecimal maxTotal = payTotal.subtract(yunfei).setScale(0, RoundingMode.DOWN);
                    resultMap.put("total1", maxTotal);
                    BigDecimal deductionPrice = BigDecimal.ZERO;
                    BigDecimal deductionPriceAll = BigDecimal.ZERO;
                    //获取积分抵扣比例
                    String scoreRateStr = getScoreRate(vo.getStoreId());
                    if (StringUtils.isNotEmpty(scoreRateStr))
                    {
                        resultMap.put("scoreRatio", scoreRateStr);
                        deductionPrice = getScoreDeductionPrice(user.getScore(), scoreRateStr);
                        deductionPriceAll = deductionPrice;
                        deductionPrice = BigDecimal.ZERO;
                        scoreDeductionValue = true;
                    }
                    resultMap.put("score", user.getScore());
                    if (Objects.nonNull(vo.getScoreDeduction()) && vo.getScoreDeduction() > 0)
                    {
                        if (user.getScore() < vo.getScoreDeduction())
                        {
                            throw new LaiKeAPIException(ErrorCode.BizErrorCode.ERROR_CODE_YHJFBZBNJXKC, "用户积分不足");
                        }
                        deductionPrice = getScoreDeductionPrice(vo.getScoreDeduction(), scoreRateStr);
                        //防止前端输入积分大于最大抵扣金额
                        if (deductionPrice.compareTo(maxTotal) > 0)
                        {
                            throw new LaiKeAPIException(ErrorCode.BizErrorCode.ERROR_CODE_CGGDDZDDKJF, "超过该订单最大抵扣积分");
                        }
                        deductionPriceAll = deductionPrice;
                        payTotal = payTotal.subtract(deductionPrice);
                    ...
                    resultMap.put("scoreDeductionPrice", deductionPrice);
                    resultMap.put("scoreDeductionPrice_all", deductionPriceAll);
                }
            }
            //供应商商品不能使用积分抵扣
            if (isGysGood == 1)
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_WLYC, "网络异常", "settlement");
        }
    ...
    public Map<String, Object> payment(OrderVo vo) throws LaiKeAPIException
    {
        ...
        try
        {
            ...
            currencyStoreModel.setCurrency_id(user.getPreferred_currency());
            currencyStoreModel.setStore_id(user.getStore_id());
            currencyStoreModel = currencyStoreModelMapper.selectOne(currencyStoreModel);

            if (currencyStoreModel == null)
            {
                //币种不存在
                throw new LaiKeAPIException(ERROR_CODE_HBBCZ, "货币不存在", "payment");
            }

            if (currencyStoreModel.getExchange_rate().compareTo(vo.getExchange_rate()) != 0)
            {
                throw new LaiKeAPIException(ERROR_CODE_HBHLYBG, "货币汇率已变更，请重新下单", "payment");
            }
            ...
            //优惠金额
            BigDecimal youhui = BigDecimal.ZERO;
            //  商品数组--------['pid'=>66,'cid'=>88]
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_CSGSBZQ, vo.getProductsInfo() + ">参数格式不正确", "payment");
            }
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_KCBZ, "库存不足1", "payment");
            }
            // 4.查询默认地址
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_QXZSHDZ, "请填写至少一个收货地址", "payment");
            }
            // 存储收货信息
            ...
            String cpc = userAddress.getCpc();
            //自提点区号，如果不是自提订单就是用户区号
            String shopCpc = cpc;
            ...
            if (shopAddressId != 0)
            {
                ...
                shopCpc = MapUtils.getString(shopMap, "cpc");
            ...
            if (vo.getIs_self_delivery() != null && vo.getIs_self_delivery() == 2)
            {
                yunfei = BigDecimal.ZERO;
                shopStatus = 2;
                storeSelfDeliveryModel.setDelivery_period(vo.getDeliveryPeriod());
                storeSelfDeliveryModel.setDelivery_time(DateUtil.dateFormateToDate(vo.getDeliveryTime(), GloabConst.TimePattern.YMD));
                storeSelfDeliveryModelMapper.insert(storeSelfDeliveryModel);
            }
            ...
            if (couponIdList.size() > 0 && isCoupon)
            {
                ...
                {
                    throw new LaiKeAPIException(ERROR_CODE_YHJYSYONG, "优惠劵已使用请重新下单", "payment");
                }
            }
            // 获取店铺的优惠券
            // 获取店铺的优惠券 是否是商品自配，此参数0为快递，1为自提，2为商家自配
            Integer is_self_driver = vo.getIs_self_delivery();
            if (is_self_driver == null)
            {
                is_self_driver = 0;
            ...
            if (Objects.nonNull(vo.getScoreDeduction()) && vo .getScoreDeduction() > 0)
            {
                if (user.getScore() < vo.getScoreDeduction())
                {
                    throw new LaiKeAPIException(ERROR_CODE_YHJFBZBNJXKC,"用户积分不足");
                }
                //积分比例
                scoreRate = getScoreRate(storeId);
                //获取抵扣金额
                BigDecimal scoreDeductionPrice = getScoreDeductionPrice(vo.getScoreDeduction(),scoreRate);

                //防止前端传入积分大于最大支付金额
                BigDecimal maxTotal = total.subtract(yunfei).setScale(0, RoundingMode.DOWN);
                if (scoreDeductionPrice.compareTo(maxTotal) > 0)
                {
                    throw new LaiKeAPIException(ERROR_CODE_JFDKCW,"积分抵扣大于最大支付金额");
                }

                productsTotal = productsTotal.subtract(mchPreferentialAmount).subtract(preferential_amount).subtract(gradeRateAmount);
                test(products,productsTotal,scoreDeductionPrice,vo.getScoreDeduction());
            }
            ...
            for (Map<String, Object> mchProduct : products)
            {
                ...
                //判断客户是不是第一次下单,是第一次下单就店铺表的新可下单数+1
                int countNum = orderModelMapper.countNewOrderNum(storeId, shopId, userId);
                if (countNum == 0)
                {
                    MchModel mchModel = mchModelMapper.selectByPrimaryKey(shopId);
                    mchModel.setNew_user_order_num(mchModel.getNew_user_order_num() + 1);
                ...
                //list中包含多个商品的全部信息，如果只有一个商品则只有list0，将这些商品信息存放在onlyProducts中
                List<Map<String, Object>> onlyProducts = (List<Map<String, Object>>) mchProduct.get("list");
                int                       index        = 0;
                ...
                if (StringUtils.isNotEmpty(vo.getScoreRatio()))
                {
                    scoreRate = getScoreRate(vo.getStoreId());
                    if (!vo.getScoreRatio().equals(scoreRate))
                    {
                        throw new LaiKeAPIException(ERROR_CODE_BLSZFSGB, "比例设置发生改变");
                    }
                ...
                for (Map<String, Object> product : onlyProducts)
                {
                    ...
                    //根据id查询商品表  新增不配送区域判断
                    ProductListModel productListModel = productListModelMapper.selectByPrimaryKey(pid);
                    int           freight          = Integer.parseInt(productListModel.getFreight());
                    if (StringUtils.isNotEmpty(vo.getFreights()))
                    {
                        String[] freights = vo.getFreights().split(SplitUtils.DH);
                        for (String data : freights)
                        {
                            String[] freightInfo = data.split(SplitUtils.MH);
                            int goodsId = Integer.parseInt(freightInfo[0]);
                            int freightId = Integer.parseInt(freightInfo[1]);
                            if (goodsId == pid)
                            {
                                if (freightId != freight)
                                {
                                    throw new LaiKeAPIException(ErrorCode.SysErrorCode.ALL_CODE,"运费发生改变，请重新进入支付");
                                }
                            }
                        }
                    }
                    ...
                    if (vo.getShopAddressId() == 0)
                    {
                        FreightModel freightModel = freightModelMapper.selectByPrimaryKey(freight);
                        if (freightModel != null)
                        {
                            ...
                            if (objects.contains(address))
                            {
                                ...
                                throw new LaiKeAPIException(ERROR_CODE_DZCCPSFW, "地址超出配送范围", "payment");
                            }
                        }
                    }
                    ...
                    Double actual_total = MapUtils.getDouble(product, "actual_total");
                    if (Objects.nonNull(actual_total))
                    {
                        orderDetailsModel.setActualTotal(BigDecimal.valueOf(actual_total));
                    }
                    Integer scoreDeduction = MapUtils.getInteger(product, "score_deduction");
                    if (Objects.nonNull(scoreDeduction))
                    {
                    ...
                    if (vo.getIs_self_delivery() != null && vo.getIs_self_delivery() == 2)
                    {
                        orderDetailsModel.setStore_self_delivery(storeSelfDeliveryModel.getId().toString());
                    }
                    //如果是供应商商品则保存此时的总供货价
   ...
   /*                 if (Objects.nonNull(vo.getScoreDeduction()))
                    {
                        if (vo.getScoreDeduction() > user.getScore() || vo.getScoreDeduction() < 0)
                        {
                            throw new LaiKeAPIException(PARAMETER_VAL_ERROR, "参数错误");
                        }
                        OrderDetailRate orderDetailRate = calcOrderDetailRate(vo.getScoreDeduction(), orderDetailsModel, total, scoreRate);
                        if (onlyProducts.size() == index)
                        {
                            if (onlyProducts.size() == 1)
                            {
                                scoreTotal = orderDetailRate.getPayScore();
                                actualTotal = orderDetailRate.getActualTotal();
                            }
                            else
                            {
                                scoreTotal = vo.getScoreDeduction() - scoreTotal;
                                actualTotal = total.subtract(actualTotal);
                            }
                        }
                        else
                        {
                            actualTotal = actualTotal.add(orderDetailRate.getActualTotal());
                            scoreTotal = scoreTotal + orderDetailRate.getPayScore();
                        }
                        orderDetailsModel.setActualTotal(actualTotal);
                        orderDetailsModel.setScoreDeduction(scoreTotal);
                    }*/
                    ...
                    if (beres < 1)
                    {
                        throw new LaiKeAPIException(ERROR_CODE_XDSBQSHZS, "下单失败,请稍后再试", "payment");
                    }
            ...
            if (Objects.nonNull(vo.getScoreDeduction()) && vo.getScoreDeduction() > 0)
            {
                BigDecimal deductionPrice = getScoreDeductionPrice(vo.getScoreDeduction(), scoreRate);
                total = total.subtract(deductionPrice);
            }
            int vipSource = 0;
            if (Objects.nonNull(vo.getVipSource()) && vo.getVipSource().equals(DictionaryConst.WhetherMaven.WHETHER_OK))
            {
                vipSource = 1;
            }
            ...
            orderModel.setVip_source(vipSource);
            orderModel.setScoreDeduction(vo.getScoreDeduction());
            ...
            orderModel.setCpc(cpc);
            orderModel.setShop_cpc(shopCpc);
...
//            orderModel.setP_sNo("");
            ...
            if (r_o >= 0)
            {
                ...
                if (vo.getGradeLevel() != 0 && vo.getGradeLevel() != null)
                {
                    ...
                    if (res_1 < 0)
                    {
                        throw new LaiKeAPIException(ERROR_CODE_XDSBGXDHQSYZTSB, "下单失败,更新兑换券使用状态失败", "payment");
                    }
                }
                if (OrderVo.SUBTRACTION.equals(discountType))
                {
...
//                this.splitOrder(vo);
    ...
    public Map<String, Object> updateOrderRemark(OrderVo vo)
    {
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_CZSB, "操作失败", "updateOrderRemark");
        }
        return null;
    }
    ...
    public Map<String, Object> splitOrder(OrderVo vo)
    {
        ...
        try
        {
            ...
            {
                // 数据异常，返回错误提示
                throw new LaiKeAPIException(ERROR_CODE_DDBCZ, sNo + "订单不存在", "payment");
            }
            //只有普通订单才需要拆单
...
//            }
            ...
            if (!CollectionUtils.isEmpty(shopIds) && shopIds.size() > 1)
            {
                ...
                for (String shopId : shopIds)
                {
                    ...
                    //查询单个商品的价格，运费，数量，如果有单店铺多商品则list数量多，在此处将店铺信息转化成订单信息
                    ...
                    if (!CollectionUtils.isEmpty(orderDetailsList))
                    {
                        ...
                        //下面这个for主要是为了计算该店铺下的单的金额和运费的相关信息，将它从总父订单中分为2个父订单，算出他们的2个父订单每个的数据
                                ...
                                for (Map<String, Object> couponInfoMap : couponList)
                                {
                                    ...
                                    if (shopId.equals(mchIdTmp))
                                    {
                                        ...
                                        if (row <= 0)
                                        {
                                            ...
                                            throw new LaiKeAPIException(ERROR_CODE_TJYHQGLDDSJSB, "添加优惠券关联订单数据失败", "splitOrder");
                                        }
...
//                        if (splitOrders.size() > 0 && splitOrders.size() == size) {
                        if (splitOrders.size() > 0 && splitOrders.size() == size)
                        {
                            int i = orderModelMapper.insertList(splitOrders);
                            if (i < 0)
                            {
                                logger.error("订单拆分失败！");
                                throw new LaiKeAPIException(ERROR_CODE_CZSB, "操作失败", "splitOrder");
                            }
                            String orderHeade = RedisDataTool.getOrderHeade(1);
                            String store_id   = String.valueOf(vo.getStoreId());

                            //redisUtil.set(orderHeade + store_id)
                ...
                {
                    Map<String, Object> splitSupplierOrder = this.splitSupplierOrder(vo.getStoreId(), supplierOrder, sNo, status);
                    if (splitSupplierOrder.containsKey("supplierOrderLists"))
                    {
                        //如果全部为供应商商品resultMap中的list为null
                        List<OrderModel> orderList = new ArrayList<>();
                        if (size != 0)
                        {
                            orderList = JSON.parseObject(JSON.toJSONString(resultMap.get("list")), new TypeReference<List<OrderModel>>()
                            {
                            });
                        }
                        //将供应商订单也加入list用于改变支付后改变订单状态
                        List<OrderModel> splitSupplierOrderList = JSON.parseObject(JSON.toJSONString(splitSupplierOrder.get("supplierOrderLists")), new TypeReference<List<OrderModel>>()
                        {
                        });
                        orderList.addAll(splitSupplierOrderList);
            ...
            //同店铺拆单
            {
                ...
                orderModelslist.add(orderModel1);
                resultMap.put("list", orderModelslist);
                //是否有供应商拆单
                boolean isSplitSupplier = false;
                //是否所有商品都是供应商商品，在同一个店铺中如果有一个不是供应商的商品则需要拆单
                boolean             isSameSupplier        = false;
                Map<String, Object> splitSupplierOrderMap = new HashMap<>();
                for (OrderDetailsModel detailsModel : orderDetailsList)
                {
                    ...
                    {
                        //供应商拆单(同店铺未拆单)
                        splitSupplierOrderMap = this.splitSupplierOrder(vo.getStoreId(), sNo);
                        //若订单中全为供应商订单则为false
                        isSplitSupplier = DataUtils.getBooleanVal(splitSupplierOrderMap, "SupplierOrder");
                        //供应商拆单了标识
                        resultMap.put("Supplier", "Supplier");
                        resultMap.put("list", (List<OrderModel>) splitSupplierOrderMap.get("supplierOrderLists"));
                        break;
                    }
                }
                /*if (isSameSupplier) {
                ...
                if (isSplitSupplier)
                {
                    ...
                    List<OrderModel> orderModelsTemplist = new ArrayList<>();
                    orderModelsTemplist.add(orderModelTmp);
                    if (splitSupplierOrderMap.containsKey("supplierOrderLists"))
                    {
                        orderModelsTemplist.addAll(JSON.parseObject(JSON.toJSONString(splitSupplierOrderMap.get("supplierOrderLists")), new TypeReference<List<OrderModel>>()
                        {
                        }));
                    }
                    resultMap.put("list", orderModelsTemplist);
                    //关联新订单
                    int row = orderDetailsModelMapper.updateOrderDetailsBysNo(storeId, orderModelTmp.getsNo(), sNo);
                    if (row < 1)
                    {
                        logger.error("修改订单号失败！");
                        throw new LaiKeAPIException(ERROR_CODE_CZSB, "操作失败");
                    }
                    //拆单后关闭并逻辑删除原主订单
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_, e.getMessage(), "splitOrder");
        }
        //操作成功
        resultMap.put("flag", true);
        return resultMap;
    ...
    public Map<String, Object> orderList(OrderVo vo)
    {
        ...
        {
            logger.error("订单列表获取失败 异常", e);
            throw new LaiKeAPIException(ERROR_CODE_DDLBHQSB, "订单列表获取失败", "orderList");
        }
    }
    ...
    public Map<String, Object> remindDelivery(OrderVo vo)
    {
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_TXFHSB, "提醒发货失败", "remindDelivery");
        }
        return retMap;
    }
    ...
    public Map<String, Object> cancleOrder(OrderVo vo)
    {
        ...
        try
        {
            ...
            if (order != null)
            {
                //生成订单时已经扣除了积分，取消订单时要返还积分
                if (Objects.nonNull(order.getScoreDeduction()) && order.getScoreDeduction() > 0)
                {
                    logger.info("积分组合支付退款流程***************************");
                    userMapper.updateUserScore(order.getScoreDeduction(), storeId, user.getUser_id());
                    AddScoreVo addScoreVo = new AddScoreVo();
                    addScoreVo.setType(SignRecordModel.ScoreType.INTEGRAL_REFUND_RETURN);
                    addScoreVo.setOrderNo(order.getsNo());
                    addScoreVo.setUserId(user.getUser_id());
                    addScoreVo.setStoreId(storeId);
                    addScoreVo.setScoreOld(user.getScore());
                    addScoreVo.setScore(order.getScoreDeduction());
                ...
                switch (status)
                {
                    ...
                    case ORDER_UNRECEIVE:
                        //已经发货不支持取消订单
                        throw new LaiKeAPIException(ERROR_CODE_YJFH, "已经发货", "cancleOrder");
                    default:
                        if (Objects.nonNull(offsetBalance) && offsetBalance.doubleValue() > 0)
                        {
                            //修改用户余额
                ...
                params.put("cancel_method", 1);
                // 228 取消订单将金额修改订单金额改为0
                //params.put("z_price", BigDecimal.ZERO);
                ...
                if (row1 >= 0 && row >= 0)
                {
                            ...
                            && !"0".equals(couponId))
                    {
                        ...
                        if (row == 0)
                        {
                            throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "cancleOrder");
                        }
                        ...
                        {
                            //回滚删除已经创建的订单
                            throw new LaiKeAPIException(ERROR_CODE_TJYHQGLDDSJSB, "添加优惠券关联订单数据失败", "cancleOrder");
                        }
                    }
                    OrderDetailsModel orderDetailsModel = new OrderDetailsModel();
                    orderDetailsModel.setStore_id(storeId);
                    ...
                    if (sNo.contains(DictionaryConst.OrdersType.ORDERS_HEADER_VI) &&
                            orderDetailsModels.get(0).getWrite_time_id() != null && orderDetailsModels.get(0).getWrite_time() != null)
                    {
                        //需要预约的商品
                        //预约的时间   2024-07-10 05:00-06:00
                        String   writeTime = orderDetailsModels.get(0).getWrite_time();
                        String[] s         = writeTime.split(" ", 2);
                        writeTime = s[0];
                        Integer            writeTimeId        = orderDetailsModels.get(0).getWrite_time_id();
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_, e.getMessage(), "cancleOrder");
        }
    }
            ...
            if (order != null)
            {
                ...
                if (row1 >= 0 && row2 >= 0)
                {
                    ...
                    for (Map<String, Object> detail : orderDetailsModelList)
                    {
                        ...
                        {
                            throw new LaiKeAPIException(OPERATION_FAILED, "当前订单正在售后处理,不可关闭");
                        }
...
/*                        if (status.equals(DictionaryConst.OrdersStatus.ORDERS_R_STATUS_COMPLETE) || status.equals(DictionaryConst.OrdersStatus.ORDERS_R_STATUS_CLOSE)) {
                ...
                {
                    throw new LaiKeAPIException(ERROR_CODE_SCSB, "删除失败", "delOrder");
                }
            }
            else
            {
                throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "delOrder");
            }
        }
        catch (Exception e)
        {
    ...
    public Map<String, Object> buyAgain(BuyAgainVo vo)
    {
        ...
        try
        {
            ...
            if (order != null)
            {
                ...
                for (OrderDetailsModel orderDetails : orderDetailsModelList)
                {
                    ...
                    if (num > 0)
                    {
                        ...
                        {
                            throw new LaiKeAPIException(ERROR_CODE_KCBCZ_001, "库存不充足", "buyAgain");
                        }
                    }
                    else
                    {
                        throw new LaiKeAPIException(ERROR_CODE_SPYSQHWSJWFGM, "商品已售罄或未上架无法购买", "buyAgain");
                    }
                }
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "buyAgain");
            }
        }
        catch (LaiKeAPIException l)
        {
    ...
    public Map<String, Object> orderSearch(OrderVo vo)
    {
        ...
        {
            logger.error("业务异常", e);
            throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "orderSearch");
        }
        return resultMap;
    }
    ...
    public Map<String, Object> delCart(OrderVo vo)
    {
        ...
        try
        {
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "delCart");
            }
    ...
    public Map<String, Object> orderDetails(OrderVo vo)
    {
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_FFRQ, "非法入侵", "orderDetails");
        }
        return resultMap;
    }
    ...
    public Map<String, Object> showLogistics(OrderVo vo)
    {
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_HQWLXXYC, "获取物流信息 异常", "getLogistics");
        }
        return map;
    }
    ...
    public Map<String, Object> cancleApply(int storeId, int id)
    {
        ...
        try
        {
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "cancleApply");
            }
        }
        catch (Exception e)
        {
    ...
    public Map<String, Object> okOrder(OrderVo orderVo)
    {
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "okOrder");
        }
        ...
        try
        {
            ...
            for (String did : detailIds)
            {
                ...
                if (orderModel == null)
                {
                    ...
                    isWholeOrderAfterSaler = (orderTotalDetailsList.size() == detailIds.size()) || orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_FS);
                }
                ...
                //限时折扣订单退款的应退金额，加上加购商品的金额
                if (orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_FS)) {
                    //应退金额加上加购商品的
                    List<OrderDetailsModel> addGoodsList = orderDetailsModelMapper.getAddGoodsList(orderModel.getsNo());
                    if (org.apache.commons.collections.CollectionUtils.isNotEmpty(addGoodsList)) {
                        double total = addGoodsList.stream()
                                .mapToDouble(order -> order.getP_price().multiply(new BigDecimal(order.getNum())).doubleValue())
                                .sum();

                       /* double addGoodsFreight = addGoodsList.stream()
                                .mapToDouble(order -> Objects.nonNull(order.getFreight()) ? order.getFreight().doubleValue() : BigDecimal.ZERO.doubleValue())
                                .sum();*/
                        refundPrice = refundPrice.add(new BigDecimal(total)).setScale(2, RoundingMode.HALF_UP);
                    }
                }
                ...
                if ((orderModel.getStatus().equals(DictionaryConst.OrdersStatus.ORDERS_R_STATUS_CONSIGNMENT) &&
                        (orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_GM) ||
                                orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_MS) ||
                                orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_FS)
                                || orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_PS)
                                || orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_PT)
                        )
                ) || (orderModel.getStatus().equals(DictionaryConst.OrdersStatus.ORDERS_R_STATUS_DISPATCHED) && orderModel.getSelf_lifting() == 1))
                {
                    //商品全部都未发货-开始极速退款流程
    ...
    public Map<String, Object> returnData(ApplyReturnDataVo vo, String imgUrl, String tokenKey) throws LaiKeAPIException
    {
        ...
        try
        {
            ...
            for (String did : detailIds)
            {
                ...
                //自提订单待收货也能急速退款
                if ((orderModel.getStatus().equals(DictionaryConst.OrdersStatus.ORDERS_R_STATUS_CONSIGNMENT) &&
                        orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_GM) ||orderModel.getOtype().equals(DictionaryConst.OrdersType.ORDERS_HEADER_MS)) || (orderModel.getStatus().equals(DictionaryConst.OrdersStatus.ORDERS_R_STATUS_DISPATCHED) && orderModel.getSelf_lifting() == 1))
                {
                    //商品全部都未发货-开始极速退款流程
                    RefundVo refundVo = new RefundVo();
                    refundVo.setStoreId(vo.getStoreId());
                    refundVo.setId(returnOrderModel.getId());
                    //计算汇率
                    if (StringUtils.isNotEmpty(orderModel.getExchange_rate()))
                    {
                        refundPrice = refundPrice.multiply(orderModel.getExchange_rate()).setScale(2, RoundingMode.HALF_UP);
                    }
                    refundVo.setPrice(refundPrice);
                    refundVo.setSNo(orderDetailsModel.getR_sNo());
                    refundVo.setType(DictionaryConst.ReturnOrderStatus.RETURNORDERSTATUS_QUICK_REFUND);
        ...
        {
            logger.error("申请售后 异常:{}", e.getMessage());
            throw new LaiKeAPIException(ERROR_CODE_WLYC, "网络异常", "returnData");
        }
        return resultMap;
    }
...
//                        }
                    ...
                    {
                        refundPrice = publicOrderService.getOrderPrice(did, storeId);
                    }
    ...
    public Map<String, Object> seeSend(int storeId, int productId)
    {
        ...
        try
        {
            ...
            if (productListModel == null)
            {
                throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "seeSend");
            }
            ...
            else
            {
                ...
                {
                    cpc = mchModel.getCpc();
                    address = CpcUtils.coverAddress(cpc,mchModel.getSheng(),mchModel.getShi(),mchModel.getXian(),mchModel.getAddress());
    ...
    public Map<String, Object> seeExtractionCode(OrderVo vo)
    {
        ...
        try
        {
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_YWYC, "业务异常", "seeExtractionCode");
            }
            ...
            for (int i = 0; i < orderCodesInfo.size(); i++)
            {
                ...
                Integer writeOffSettings = MapUtils.getInteger(productMap, "write_off_num");
                if (Objects.nonNull(writeOffSettings))
                {
                    productRetMap.put("status",writeOffSettings != 0 ? 1 : 2);
                }
    ...
    public Map<String, Object> returndetails(RefundDetailsVo refundDetailsVo)
    {
        ...
        try
        {
            ...
            goodsInfo.put("price", MapUtils.getDouble(returnOrder, "p_price"));
...
//                orderDetailsModel1.setR_sNo(sNo);
            ...
            else
            {
                ...
                throw new LaiKeAPIException(ERROR_CODE_SHQRSHYC, "售后确认收货异常", "confirmReceipt");
            }
    ...
    public Map<String, Object> cancelApplication(CancleAfterSaleApplyVo vo)
    {
        ...
        try
        {
            ...
            {
                throw new LaiKeAPIException(ERROR_CODE_ZTFSBH, "状态发生变化");
            }
            //虚拟商品撤销审核时需要返还退款的核销次数
            if (returnOrderModel.getsNo().contains(DictionaryConst.OrdersType.ORDERS_HEADER_VI))
            {
                OrderDetailsModel orderDetailsModel = orderDetailsModelMapper.selectByPrimaryKey(returnOrderModel.getP_id());
                if (returnOrderModel.getR_write_off_num() != null && returnOrderModel.getR_write_off_num() > 0)
                {
                    orderDetailsModel.setWrite_off_num(returnOrderModel.getR_write_off_num());
                    int i = orderDetailsModelMapper.updateByPrimaryKey(orderDetailsModel);
                    if (i < 1)
                    {
                        logger.error("撤销审核失败失败！");
                        throw new LaiKeAPIException(ERROR_CODE_CXSHSB, "撤销审核失败", "cancelApplication");
                    }
                }
            }
    ...
    @Transactional(rollbackFor = Exception.class)
    public Map<String, Object> splitSupplierOrder(int storeId, String orderNo) throws LaiKeAPIException
    {
        ...
        try
        {
            ...
            if (orderOld == null)
            {
                throw new LaiKeAPIException(ERROR_CODE_CZSB, "操作失败", "splitSupplierOrder");
            }
            //拆单后的供应商订单集合
            List<OrderModel> supplierOrderLists = new ArrayList<>();
            //支付总金额
            ...
            if (orderDetailsModelMapper.selectCount(orderDetailsCount) <= 1)
            {
                ...
                SupplierOrder = false;
                supplierOrderLists.add(orderOld);
                result.put("SupplierOrder", SupplierOrder);
                result.put("supplierOrderLists", supplierOrderLists);
                return result;
            }
            //供应商订单集合
            ...
            if (supplierOrderList.size() > 0)
            {
                for (Map<String, Object> map : supplierOrderList)
                {
                    ...
                    supplierOrderLists.add(orderModelTmp);
                    //关联新订单
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_, e.getMessage(), "splitSupplierOrder");
        }
    }

    @Transactional(rollbackFor = Exception.class)
    public Map<String, Object> splitSupplierOrder(int storeId, Map<Integer, List<Map<String, Object>>> mapList, String sNo, Integer status) throws LaiKeAPIException
    {
        Map<String, Object> result = new HashMap<>();
        try
        {
    ...
    public void supplierOrderFreight(int storeId, String sNo) throws LaiKeAPIException
    {
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_, e.getMessage(), "supplierOrderFreight");
        }
    }
    ...
    public Map<String, Object> getPayment(OrderVo vo) throws LaiKeAPIException
    {
        ...
        catch (Exception e)
        {
            ...
            throw new LaiKeAPIException(ERROR_CODE_, e.getMessage(), "getPayment");
        }
     ...
     * @return
     */
    private BigDecimal getScoreDeductionPrice(Integer score, String scoreRateStr)
    {
        String[] split = scoreRateStr.split(":");
        //积分
        int scoreRate = Integer.parseInt(split[0]);
        //金额
        int    priceRate      = Integer.parseInt(split[1]);
        double fullDeductions = Arith.div(score, scoreRate);
        return BigDecimal.valueOf(Arith.mul(fullDeductions, priceRate));
    }
     ...
     * @return
     */
    private String getScoreRate(Integer storeId)
    {
        ...
        integralConfigModel.setStatus(1);
        integralConfigModel = integralConfigModelMapper.selectOne(integralConfigModel);
        if (Objects.nonNull(integralConfigModel) && StringUtils.isNotEmpty(integralConfigModel.getScoreRatio()))
        {
            return integralConfigModel.getScoreRatio();
        }
        return "";
    }

    @Data
    public static class OrderDetailRate
    {
    ...
    /**
     * @param products 商品数据
     * @param productsTotal         商品总价
     * @param deductionPrice        抵扣总金额
     * @param payScore                 抵扣总积分
     */
    public void test(List<Map<String,Object>> products, BigDecimal productsTotal, BigDecimal deductionPrice, Integer payScore)
    {
        // 累计抵扣金额
        BigDecimal deductionTotalPrice = BigDecimal.ZERO;
        // 累计抵扣积分
        Integer totalScore = 0;
        //商品最后一个数量
        int proNum = products.size() - 1;
        int i = 0;
        for (Map<String, Object> prodMap : products)
        {

            List<Map<String, Object>> productList = (List<Map<String, Object>>) prodMap.get("list");
            int                       proNum1     = productList.size() - 1;
            int j = 0;
            for (Map<String, Object> map : productList)
            {

                //详单优惠后金额
                BigDecimal  afterDiscount       = BigDecimal.valueOf(MapUtils.getDouble(map, "amount_after_discount"));
                BigDecimal scoreDeductionPrice = BigDecimal.ZERO;
                int scoreDeduction = 0;
                //最后一个店铺&&最后一个商品
                if (proNum1 == j && proNum == i)
                {
                    scoreDeductionPrice = deductionPrice.subtract(deductionTotalPrice).setScale(2, RoundingMode.HALF_UP);
                    scoreDeduction = payScore - totalScore;
                }
                else
                {
                    //详单抵扣金额 = 该单优惠后金额 / 商品总价 * 需要抵扣的金额
                    scoreDeductionPrice = afterDiscount.divide(productsTotal, 2, RoundingMode.HALF_UP)
                            .multiply(deductionPrice)
                            .setScale(2, RoundingMode.HALF_UP);

                    //详单抵扣积分 = 该单优惠后金额 / 商品总价 * 需要抵扣的积分
                    scoreDeduction = afterDiscount.divide(productsTotal,2,RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(payScore)).intValue();
                    // 累计抵扣金额和积分
                    deductionTotalPrice = deductionTotalPrice.add(scoreDeductionPrice);
                    totalScore += scoreDeduction;
                }
                //详情实际支付金额
                map.put("actual_total",scoreDeductionPrice);
                //详单实际抵扣积分
                map.put("score_deduction",scoreDeduction);
                j++;
            }
            i++;
        }
