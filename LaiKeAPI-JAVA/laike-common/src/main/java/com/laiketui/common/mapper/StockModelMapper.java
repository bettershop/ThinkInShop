...
public interface StockModelMapper extends BaseMapper<StockModel>
{
     ...
     * @date 2021/1/4 10:36
     */
    Integer stockInfoCountDynamic(Map<String, Object> map) throws LaiKeAPIException;
     ...
     * @date 2021/1/4 10:36
     */
    Integer goodsStockInfoCountDynamic(Map<String, Object> map) throws LaiKeAPIException;
     ...
     */
    @Select(" SELECT stock.topClass, stock.className, IF( stock.sy_num IS NULL, 0, stock.sy_num) AS sy_num, IF( stock.in_num IS NULL, 0, stock.in_num) AS in_num, IF ( stock.out_num IS NULL, 0, stock.out_num ) AS out_num FROM ( SELECT a.cid AS topClass, a.pname AS className, SUM(b.num) AS sy_num, ( SELECT SUM(flowing_num) FROM lkt_stock x, lkt_product_list y WHERE x.store_id = b.store_id AND x.product_id = y.id AND x.type = 0 AND x.product_id = y.id AND y.recycle = 0 and y.is_presell=0 and y.status=2 AND substring_index( substring_index(y.product_class, '-', 2), '-', - 1 ) = a.cid ) AS in_num, ( SELECT SUM(flowing_num) FROM lkt_stock x, lkt_product_list y WHERE x.store_id = b.store_id AND x.product_id = y.id AND x.type = 1 AND x.product_id = y.id AND y.recycle = 0 and y.is_presell=0 and y.status=2 AND substring_index( substring_index(y.product_class, '-', 2), '-', - 1 ) = a.cid ) AS out_num FROM lkt_product_class a LEFT JOIN lkt_product_list b ON b.store_id = a.store_id AND b.recycle = 0 AND a.cid = substring_index( substring_index(b.product_class, '-', 2), '-', - 1 ) WHERE a.store_id = #{storeId} AND a.`level` = 0 AND a.recycle = 0 and b.is_presell=0 and b.status=2 GROUP BY topClass ORDER BY a.cid DESC ) stock ")
    ...
    Long countWithParam(int storeid, String date, String type);

    Long countWarnAmount(int storeid, String date);
}