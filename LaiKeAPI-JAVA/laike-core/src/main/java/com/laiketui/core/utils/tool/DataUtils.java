...
public final class DataUtils
{
     ...
     * @date 2021/3/11 10:31
     */
    public static <T> T mapSort(Map<Integer, ?> map, Sort sort, Class<T> cls) throws LaiKeAPIException
    {
        try
        {
            Map<Integer, Object> resultMap = new LinkedHashMap<>(16);
            if (map == null)
            {
                return null;
            }
            List<Integer> number = new ArrayList<>();
            map.forEach((key, val) -> number.add(key));

            switch (sort.value)
            {
                case Constant.DESC:
                    //从大至小排序
                    number.sort((o1, o2) -> o1 > o2 ? -1 : 0);
                    break;
                case Constant.ASC:
                    //从小至大排序
                    number.sort((o1, o2) -> o1 >= o2 ? 0 : -1);
                    break;
                default:
                    break;
            }
            number.forEach(key -> resultMap.put(key, map.get(key)));
            return JSON.parseObject(JSON.toJSONString(resultMap), cls);
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    public static double random(double min, double max, boolean isInt) throws LaiKeAPIException
    {
        ...
        {
            logger.error("生成随机金额异常: min={}, max={}", min, max, e);
            throw new LaiKeCommonException("生成随机金额失败: " + e.getMessage());
        }
    }
