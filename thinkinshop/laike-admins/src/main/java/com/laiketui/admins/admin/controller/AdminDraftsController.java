...
@Api(tags = "草稿箱管理")
@RestController
@RequestMapping("/admin/drafts")
public class AdminDraftsController
{
    @Autowired
    ...
    @ApiOperation("/新增草稿箱")
    @PostMapping("/add")
    @HttpApiMethod(apiKey = "admin.Drafts.add")
    public Result add(MainVo vo,String text,Integer id){
        try
        {
            draftsModelService.add(vo,1,text,id);
    ...
    @ApiOperation("/草稿箱列表")
    @PostMapping("/list")
    @HttpApiMethod(apiKey = "admin.Drafts.get_list")
    public Result list(MainVo vo){
        try
        {
            Map<String, Object> list = draftsModelService.list(vo, 1);
    ...
    @ApiOperation("/草稿箱删除")
    @PostMapping("/del")
    @HttpApiMethod(apiKey = "admin.Drafts.del")
    ...
    @HttpApiMethod(apiKey = "admin.Drafts.edit_page")
