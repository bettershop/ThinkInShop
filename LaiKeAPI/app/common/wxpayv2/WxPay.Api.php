...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(self::$config['notify_url']);//异步通知url
		}
		
		$inputObj->SetAppid(self::$config['appid']);//公众账号ID
		$inputObj->SetMch_id(self::$config['mch_id']);//商户号
		$inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip	  
		//$inputObj->SetSpbill_create_ip("1.1.1.1");  	    
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		
		//签名
		$inputObj->SetSign(self::$config);
		$xml = $inputObj->ToXml();
		...
		$result = WxPayResults::Init($response,self::$config);
		...
		$result = WxPayResults::InitFromArray($prePayParams,true,self::$config)->GetValues();
	...
	public static function refund($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetSign(self::$config);//签名
		$xml = $inputObj->ToXml();
		$startTimeStamp = self::getMillisecond();//请求开始时间
