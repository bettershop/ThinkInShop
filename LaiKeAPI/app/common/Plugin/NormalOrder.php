...
class NormalOrder 
{
    ...
    public function settlement(&...$context)
    {
        ...
        //4.计算运费
        $freight = Tools::get_freight($products_freight, $products, $address, $store_id, $product_type,$is_self_delivery);
        ...
        $is_self_delivery = $shop['is_self_delivery']; // 是否支持商家自配 0.否 1.是
        ...
        $Calculate_the_deduction_data_for_the_detailed_invoice = array('products' => $products,'products_total' => ($products_total - $mch_preferential_amount - $preferential_amount),'scoreDeductionPrice'=>$scoreDeductionPrice,'scoreDeductionpoints'=>$scoreDeduction);
    ...
    public function payment(&...$context)
    {
        ...
        // 6.计算运费
        $freight = Tools::get_freight($products_freight, $products, $address, $store_id, $type,$is_self_delivery);
        $products = $freight['products'];
        ...
        {
            $data_self_delivery_info = array('delivery_time'=>$delivery_time,'delivery_period'=>$delivery_period,'phone'=>'','courier_name'=>'');
            $store_self_delivery = Db::name('self_delivery_info')->insertGetId($data_self_delivery_info);
        }
        ...
        {   
            ...
            $mch_id2 = ',' . $mch_id1 . ',';
            // 根据商城id、店铺ID、user_id，查询订单
            $sql_mo = "select id from lkt_order where store_id = '$store_id' and mch_id = '$mch_id2' and user_id = '$user_id' ";
            $r_mo = Db::query($sql_mo);
            if(empty($r_mo))
            { // 不存在，新客户
                $sql_mo0 = "update lkt_mch set new_user_order_num = new_user_order_num + 1 where store_id = '$store_id' and id = '$mch_id2' ";
                $r_mo0 = Db::execute($sql_mo0);
            }
    ...
    {   
        ...
        if (empty($config))
        {
            ...
            $this->Log(__METHOD__ . ":" . __LINE__ . "普通订单执行日期：" . date('Y-m-d H:i:s') . "\n支付暂未配置 商城ID：$store_id ，支付类型：$pay_type ，无法调起支付！\r\n");
            return 'file';
        }
    ...
    {   
        ...
        if (floatval($z_price) != floatval($total))
        {
            ...
            $this->Log(__METHOD__ . ":" . __LINE__ . "【普通订单付款金额有误】:\n 应付金额为$z_price \n");
        }
        ...
        if ($r)
        {
            ...
            if ($status < 1)
            {
                ...
                $this->Log(__METHOD__ . ":" . __LINE__ . "【普通订单data】:\n" . json_encode((array)$r[0]) . "\n");
            }
        }
    }
