 ...
 * 方法三：
 *         LaiKeLogUtils::log("app/xx.log","日志内容");
 */
class LaiKeLogUtils
{
     ...
     * 在指定文件中写入日志
     */
    public static function log($logfilepath, $msg)
    {
        try
        {
            if (empty($logfilepath))
            {
                $logfilepath = LaiKeLogUtils::COMMONLOGPATH;
            }
            self::baseLog(LaiKeLogUtils::LOGROOTPATH . $logfilepath, $msg);
        } catch (Exception $e)
        {
            throw $e;
        }
    ...
    {   
        ...
        if(is_dir($path))
        {
            ...
            if(is_array($file_arr))
            {
                ...
                //判断文件数量是否超限
                if(count($list) > LaiKeLogUtils::LOGNUM)
                {
                ...
                if (count($list) > 0) 
                {
                    if($list[count($list)-1]['size'] >= LaiKeLogUtils::LOGSIZE)
                    {   
