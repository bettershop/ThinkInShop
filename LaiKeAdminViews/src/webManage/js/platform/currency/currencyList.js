...
export default {
  ...
  methods: {
    ...
    Delete(id) {
      ...
      let paramIds = id;
      let datas = {
        api: 'admin.currency.delCurrency'
      };
      if (isEmpty(id))
      {
      ...
      this.$confirm(this.$t('currency.qrsc'), this.$t('currency.ts'), {
        confirmButtonText: this.$t('currency.okk'),
        cancelButtonText: this.$t('currency.ccel'),
        type: 'warning'
      }).then(() => {
        this.LaiKeCommon.del(datas).then(res => {
          if(res.data.code == '200') {
            this.loadData();
            if(res.data.data != ''){
              this.errorMsg(res.data.data  + this.$t('bzybdsc'));
            }else{
              this.$message({
                type: 'success',
