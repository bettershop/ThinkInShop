import { secLabelGoodsList, addPro, getProAttrList } from '@/api/plug_ins/seckill'

export default {
    name: 'editorGoods',

    data() {
        return {
            pltext: "",
            ruleForm: {
                price_type: 0,
                price: '',
                activity_time: []
            },

            rules: {
                price: [
                    { required: true, message: '请填写秒杀价格', trigger: 'blur' }
                ],
                activity_time: [
                    { required: true, message: '请选择活动时间', trigger: 'change' }
                ],
            },

            goodsIds: '',

            tableData: [],

            priceList: [
                {
                    value: 0,
                    name: '百分比'
                },
                {
                    value: 1,
                    name: '固定值'
                },
            ],

            text: '%',

            // 多规格弹框数据
            dialogVisible3: false,
            ProList: [],

            goodsJson: [],
            goodsJson1: [],

            goodsNum: null,
            goodsId: null,
            attr: [],

            isTag: false

        }
    },

    created() {
        this.secLabelGoodsLists().then(() => {
            // this.getProAttrLists()
            this.isTag = true
        })

    },

    watch: {
        'ruleForm.price_type': {
            handler: function () {
                if (this.ruleForm.price_type == 0) {
                    this.text = '%'
                    if (this.ruleForm.price >= 100) {
                        this.ruleForm.price = 99
                    }
                } else {
                    this.text = '元'
                    if (this.isTag) {
                        this.ruleForm.price = 0
                    }

                }
            }
        },

        'ruleForm.price': {
            handler: function () {
                if (this.ruleForm.price_type == 1) {
                    if (parseInt(this.ruleForm.price) > parseInt(this.tableData[0].goodsPrice)) {
                        this.ruleForm.price = this.tableData[0].goodsPrice
                    }
                } else {
                    if (this.ruleForm.price >= 100) {
                        this.ruleForm.price = 99
                    } else if (this.ruleForm.price < 0) {
                        this.ruleForm.price = 1
                    }
                }
            }
        }
    },

    beforeRouteLeave(to, from, next) {
        let that = this
        console.log(to)
        if (to.path == "/plug_ins/seckill/goodsList") {
            // debugger
            console.log(that.$route.query.id)
            to.query.id = that.$route.query.labelId
        }
        next()
    },

    methods: {
        async secLabelGoodsLists() {
            const res = await secLabelGoodsList({
                api: 'plugin.sec.AdminSec.secLabelGoodsList',
                labelId: this.$route.query.labelId,
                secGoodsId: this.$route.query.id
            })
            console.log(res);
            this.tableData = res.data.data.goodsList
            this.ruleForm.price_type = this.tableData[0].price_type
            this.ruleForm.price = this.tableData[0].price
            this.ruleForm.activity_time = [this.conversionTime(this.tableData[0].starttime), this.conversionTime(this.tableData[0].endtime)]
            this.goodsIds = this.tableData[0].goodsId,
                this.id = this.tableData[0].id

        },

        tableHeaderColor({ row, column, rowIndex, columnIndex }) {
            if (rowIndex === 0) {
                return 'background-color: #f4f7f9;'
            }
        },

        conversionTime(val) {
            const d = new Date(val); // val 为表格内取到的后台时间
            const month = d.getMonth() + 1 < 10 ? `0${d.getMonth() + 1}` : d.getMonth() + 1;
            const day = d.getDate() < 10 ? `0${d.getDate()}` : d.getDate();
            const hours = d.getHours() < 10 ? `0${d.getHours()}` : d.getHours();
            const min = d.getMinutes() < 10 ? `0${d.getMinutes()}` : d.getMinutes();
            const sec = d.getSeconds() < 10 ? `0${d.getSeconds()}` : d.getSeconds();
            const times = `${d.getFullYear()}-${month}-${day} ${hours}:${min}:${sec}`;
            return times;
        },

        onInputBlur() {
            if (parseInt(this.ruleForm.price) <= 0) {
                this.ruleForm.price = 1
            }
        },

        // 获取规格信息
        async getProAttrLists() {
            const res = await getProAttrList({
                api: 'plugin.sec.AdminSec.getProAttrList',
                goodsId: this.tableData[0].goodsId,
                acId: this.$route.query.id
            })

            this.ProList = res.data.data.list.map(item => {
                return {
                    ...item,
                    addNum: 0,
                    upNum: item.secMaxNum
                }
            })

            this.goodsId = this.ProList[0].goodsId
            this.attr = this.ProList.map(item => {
                return {
                    attrId: item.id,
                    num: 0
                }
            })

            this.goodsNum = 0

            this.goodsJson1.push({
                goodsId: this.goodsId,
                goodsNum: this.goodsNum,
                attr: this.attr
            })

            console.log(this.goodsJson)

            this.goodsNum = null
            this.goodsId = null
            this.attr = []
            console.log(res);
        },

        // 多规格弹框方法
        async specifications(value) {
            this.dialogVisible3 = true
        },

        AddProconfirm() {
            this.goodsJson = []
            this.goodsId = this.ProList[0].goodsId
            this.attr = this.ProList.map(item => {
                return {
                    attrId: item.id,
                    num: item.upNum
                }
            })

            this.attr.filter(item => {
                this.goodsNum += parseInt(item.num)
            })

            this.goodsJson.push({
                goodsId: this.goodsId,
                goodsNum: this.goodsNum,
                attr: this.attr
            })

            console.log(this.goodsJson)

            this.goodsNum = null
            this.goodsId = null
            this.attr = []
            this.dialogVisible3 = false
        },

        submitForm(formName) {
            this.$refs[formName].validate(async (valid) => {
                console.log(this.ruleForm);
                if (valid) {
                    try {
                        if (this.ruleForm.price_type == 0 && this.ruleForm.price == 0) {
                            this.$message({
                                message: '百分比不能为零',
                                type: 'error',
                                offset: 100
                            })
                            return
                        }
                        this.ProList.forEach(item => {
                            this.goodsJson.forEach(item2 => {
                                if (item.goodsId == item2.id) {
                                    item2.attr.forEach(item3 => {
                                        if (item3.attrId == item.id) {
                                            item3.num = item.upNum
                                        }
                                    })
                                }
                            })
                        })
                        console.log('419419419  upNum', this.goodsJson, this.ProList);
                        if (this.goodsJson.length > 0) {
                            this.goodsJson.forEach(v => {
                                this.$set(v, 'attrId', v.attr[0].attrId)
                                delete v.attr
                            })
                        } else {
                            this.goodsJson1.forEach(v => {
                                this.$set(v, 'attrId', v.attr[0].attrId)
                                delete v.attr
                            })
                        }
                        console.log('goodsJson', this.goodsJson)
                        console.log('goodsJson1', this.goodsJson1)
                        addPro({
                            api: 'plugin.sec.AdminSec.addPro',
                            id: this.$route.query.labelId,
                            secGoodsId: this.id,
                            priceType: this.ruleForm.price_type,
                            price: this.ruleForm.price,
                            startDate: this.ruleForm.activity_time[0],
                            endDate: this.ruleForm.activity_time[1],
                            // goodsIds: this.goodsIds,
                            goodsJson: this.goodsJson.length > 0 ? encodeURIComponent(JSON.stringify(this.goodsJson)) : encodeURIComponent(JSON.stringify(this.goodsJson1)),
                        }).then(res => {
                            if (res.data.code == '200') {
                                this.$message({
                                    message: '编辑成功',
                                    type: 'success',
                                    offset: 100
                                })
                                this.$router.go(-1)
                            }
                        })
                    } catch (error) {
                        this.$message({
                            message: error.message,
                            type: 'error',
                            showClose: true
                        })
                    }
                } else {
                    console.log('error submit!!');
                    return false;
                }
            });
        },

        oninput(num, limit) {
            var str = num
            var len1 = str.substr(0, 1)
            var len2 = str.substr(1, 1)
            //如果第一位是0，第二位不是点，就用数字把点替换掉
            if (str.length > 1 && len1 == 0 && len2 != ".") {
                str = str.substr(1, 1)
            }
            //第一位不能是.
            if (len1 == ".") {
                str = ""
            }
            //限制只能输入一个小数点
            if (str.indexOf(".") != -1) {
                var str_ = str.substr(str.indexOf(".") + 1)
                if (str_.indexOf(".") != -1) {
                    str = str.substr(0, str.indexOf(".") + str_.indexOf(".") + 1)
                }
            }
            //正则替换
            str = str.replace(/[^\d^\.]+/g, '') // 保留数字和小数点
            if (limit / 1 === 1) {
                str = str.replace(/^\D*([0-9]\d*\.?\d{0,1})?.*$/, '$1') // 小数点后只能输 1 位
            } else {
                str = str.replace(/^\D*([0-9]\d*\.?\d{0,2})?.*$/, '$1') // 小数点后只能输 2 位
            }

            if (this.ruleForm.price_type == 1 && this.totlePrice !== 0 && parseInt(this.ruleForm.price) > this.totlePrice) {
                str = this.totlePrice
            }
            return str
        },

        oninput2(num) {
            var str = num
            str = str.replace(/[^\.\d]/g, '');
            str = str.replace('.', '');

            return str
        },
    }
}