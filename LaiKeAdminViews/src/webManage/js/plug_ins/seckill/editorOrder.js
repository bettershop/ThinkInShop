    ...
    methods: {
        async editOrderViews() {
            const res = await editOrderView({
                api: 'plugin.sec.order.editOrderView',
                // api: 'plugin.auction.order.editOrderView',
            ...
            this.ruleForm.remarks = this.dataInfo.remarks ? this.dataInfo.remarks : ''
        ...
        submitForm(formName) {
            this.$refs[formName].validate(async (valid) => {
              ...
              if (valid) {
                try {
                    saveEditOrder({
                        api: 'plugin.sec.order.saveEditOrder',
                        // api: 'plugin.auction.order.saveEditOrder',
                        ...
                        if(res.data.code == '200') {
                            this.$message({
                                message: this.$t('orderDetails.cg'),
                                type: 'success',
                                offset:100
                            })
                            this.$router.go(-1)
