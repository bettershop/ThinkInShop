...
export default {
    ...
    data() {
        return {
            ...
            seckillStatusList: [
                {
                    value: 1,
                    label: this.$t('seckill.goodsList.wks')
                },
                {
                    value: 2,
                    label: this.$t('seckill.goodsList.jxz')
                },
                {
                    value: 3,
                    label: this.$t('seckill.goodsList.yjs')
                }
            ],// 秒杀状态
    ...
    created() {
        if(!this.$route.query.id) {
            this.$router.push({
                path: '/plug_ins/seckill/seckillLabel',
            })
            return
        }
    ...
    methods: {
        async secLabelGoodsLists() {
            const res = await secLabelGoodsList({
                api: 'plugin.sec.AdminSec.secLabelGoodsList',
        ...
        delAll() {
            this.$confirm(this.$t('seckill.goodsList.qdsc'), this.$t('seckill.ts'), {
                confirmButtonText: this.$t('seckill.okk'),
                cancelButtonText: this.$t('seckill.ccel'),
                type: 'warning'
              }).then(() => {
                delPro({
                      api: 'plugin.sec.AdminSec.delPro',
                      id: this.idList
                  }).then(res => {
                      console.log(res);
        ...
        switchs(value) {
            labelGoodsSwitch({
                api: 'plugin.sec.AdminSec.labelGoodsSwitch',
                id: value.id
            }).then(res => {
                if(res.data.code == '200') {
                    this.$message({
                        type: 'success',
                        message: value.is_show == 0 ? this.$t('zdata.gbcg') : this.$t('zdata.kqcg'),
                        offset: 100
                    })
                }
                this.secLabelGoodsLists()
        ...
        Delete(value) {
            this.$confirm(this.$t('seckill.goodsList.qdscs'), this.$t('seckill.ts'), {
                confirmButtonText: this.$t('seckill.okk'),
                cancelButtonText: this.$t('seckill.ccel'),
                type: 'warning'
              }).then(() => {
