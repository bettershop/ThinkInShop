...
export default {
  ...
  methods: {
    ...
    async loadData() {
      await index({
        api: 'plugin.presell.AdminPreSell.orderSettlement',
        ...
        if (!isEmpty(data)) {
          ...
          data.data.data.list.forEach((item, index)=>{
            if(item.settlement_status == 1){
              item.status_name = this.$t('auction.orderSettlementList.yjs')
            }else{
              item.status_name = this.$t('auction.orderSettlementList.djs')
            }
          })
    ...
    // 重置
    reset() {
      this.search = {orderNo: null, type: null, time: null,mchName: null,};
    },
    See(orderNo) {
      this.$router.push({
        path: '/plug_ins/preSale/orderSettlementDetail',
        query: {
          orderNo: orderNo
        },
      })
