import { index } from "@/api/order/orderSettlement";
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      await index({
        // api: 'plugin.sec.order.settlement.index',
        api: 'plugin.auction.Order.settlementindex',
    ...
    See(orderNo) {
      this.$router.push({
        path: '/plug_ins/auction/orderSettlementDetail',
        query: {
          orderNo: orderNo
        },
      })
    ...
    async Del(id) {
      this.$confirm(this.$t('auction.orderSettlementList.scts'), this.$t('auction.ts'), {
        confirmButtonText: this.$t('auction.okk'),
        cancelButtonText: this.$t('auction.ccel'),
        type: 'warning'
      }).then(() => {
        del({
          // api: 'plugin.sec.order.settlement.del',
          api: 'plugin.auction.Order.delSettlement',
          id: id
        }).then(res => {
          this.demand();
          this.$message({
