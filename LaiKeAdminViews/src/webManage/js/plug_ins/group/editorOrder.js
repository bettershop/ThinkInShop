  ...
  methods: {
    async editOrderViews() {
      const res = await editOrderView({
        api: 'plugin.group.AdminGroupOrder.EditOrderView',
      ...
      this.courierList = this.dataInfo.courierList
        ; (this.ruleForm.name = res.data.data.data.name),
          ...
          (this.ruleForm.remarks =
            Object.keys(res.data.data.data.remarks).length === 0 ? '' : '')
      ...
      this.$refs[formName].validate(async valid => {
        ...
        if (valid) {
          try {
            ...
            this.isTag = false
            saveEditOrder({
              api: 'plugin.group.AdminGroupOrder.SaveEditOrder',
              ...
              orderAmt:
                this.dataInfo.status == '待付款'
                  ? this.ruleForm.pay_price
                  : null
            }).then(res => {
              console.log(res)
