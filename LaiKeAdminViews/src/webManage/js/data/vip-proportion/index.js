  ...
  methods: {
    async loadData () {
      const res = await index({
        ...
        startDate: isEmpty(this.page.inputInfo.date)
          ? ''
          : this.page.inputInfo.date[0],
        endDate: isEmpty(this.page.inputInfo.date)
          ? ''
          : this.page.inputInfo.date[1]
      })
      let dates = []
    ...
    loadLine (data) {
      let option = {
        ...
        color: ['#F8DD29', '#F49610', '#F30000', '#574326', '#987367'],
        series: [
          {
            name: this.$t('zdata.fwly'),
            type: 'pie',
            radius: ['50%', '70%'],
            ...
            data: [
              ...
              { value: data.num_h5, name: 'H5' },
              { value: data.num_zfb, name: '支付宝小程序' }
            ]
          }
        ]
      }
