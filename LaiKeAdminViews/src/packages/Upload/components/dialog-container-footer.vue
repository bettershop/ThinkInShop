    ...
    <el-footer height="72px">
      ...
      >
                ...
                <el-row align="top" type="flex">
                  <el-col>
                    <h4 style="line-height: 12px;">
                      {{$t('uploadImage.yixuan') }} {{ selectList.length }}个
                    </h4>
                  </el-col>
                </el-row>
                <el-row align="bottom">
        ...
        <div>
          <div style="display:flex;flex-direction: row-reverse">
            <div>
              <el-button class="move-class" @click="openDialog" :disabled="selectList.length <= 0">{{$t('uploadImage.yzfl')}}</el-button>
              <el-button type="primary" :disabled="selectList.length <= 0" @click="$emit('add')"
                >{{$t('uploadImage.qdtj')}}</el-button
              >
            </div>
          </div>
        </div>
      ...
      >
        <el-form
          ...
          label-width="80px"
        >
          <el-form-item :label="$t('uploadImage.flmc')" prop="group_id">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="onMoveClassify">{{ $t('uploadImage.qd') }}</el-button>
        </div>
      </el-dialog>
    </el-footer>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        group_id: [{ required: true, message:this.$t('uploadImage.qxzfl'), trigger: "blur" }]
      }
    };
  },
  computed: {
  ...
  methods: {
    ...
    onMoveClassify() {
      this.$refs.ruleForm.validate(async valid => {
        ...
        this.$message({
          message: this.$t('uploadImage.ydcg'),
          type: "success",
          offset:120
        });
        this.$emit("move");
