  ...
  <div class="dialog-left-aside">
    <el-aside class="aside" style="width: 160px;">
      ...
      <el-menu :default-active="urlAddActive" @select="onHandleAddUrlSelect">
        <el-menu-item index="addUrl">
          <span slot="title">{{ $t('uploadImage.curltj') }}</span>
        </el-menu-item>
      </el-menu>
      <div class="gap"></div>
      <el-menu
    ...
    >
      <el-form
        ...
        label-width="80px"
      >
        <el-form-item :label="$t('uploadImage.flmc')" prop="name">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          ...
          "
          >{{ $t('uploadImage.qx') }}</el-button
        >
        <el-button type="primary" class="sure" @click="createClassify">{{ $t('uploadImage.qd') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: this.$t('uploadImage.qsrflmc'), trigger: "blur" },
          { min: 1, max: 10, message: this.$t('uploadImage.cdzdgzf'), trigger: "blur" }
        ]
      }
    };
  },
  ...
  computed: {
    topClassify() {
      ...
      let obj = {
        id: -1,
        name: this.$t('uploadImage.quanbu')
      }
  ...
  methods: {
    async createClassify() {
      this.$refs.ruleForm.validate(async valid => {
        ...
        api.createClassify(this.ruleForm.name).then(res => {
          ...
          this.getClassifys()
          this.$message({
            message: this.$t('uploadImage.cjcg'),
            type: 'success',
            offset: 100
          })
        })
    ...
    async getFileList() {
      if(this.classifyActive ){
        this.pid = this.classifyActive;
      }
    ...
    async onHandleItemDelete(e, index) {
      ...
      try {
        await this.$confirm(this.$t('uploadImage.sfsccfl'), this.$t('uploadImage.ts'), {
          confirmButtonText: this.$t('uploadImage.qd'),
          cancelButtonText: this.$t('uploadImage.qx'),
          type: "warning"
        });
