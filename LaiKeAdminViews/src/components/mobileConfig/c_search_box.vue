  ...
  <div class="mobile-config">
    <Form ref="formInline">
      <div v-for="(item,key) in rCom" :key="key">
        <component :is="item.components.name" :configObj="configObj" ref="childData" :configNme="item.configNme" :key="key" @getConfig="getConfig" :index="activeIndex" :num="item.num" :imgWidth="item.imgWidth" :imgHeight="item.imgHeight" :fieldName="item.fieldName"></component>
      </div>
...
export default {
  ...
  methods: {
    ...
    getConfig(data) {
      if (data == 0) {
        if (this.rCom[1].configNme === 'logoConfig') {
