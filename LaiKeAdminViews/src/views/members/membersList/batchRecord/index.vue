<template>
          ...
          <el-table-column prop="status" :label="$t('bulkDeliveryRecord.wjzt')">
            <template slot-scope="scope">
              <span
                ...
                >{{ scope.row.status == 0 ? $t('bulkDeliveryRecord.drsb'): $t('bulkDeliveryRecord.drcg') }}</span
              >
            </template>
          </el-table-column>
    ...
    <script>
  import { deliverList, delDelivery } from '@/api/members/membersList'
    ...
    data() {
      return {
        fileStatus: [
          {
            value: "0",
            label: this.$t('bulkDeliveryRecord.drsb'),
          },
          {
            value: "1",
            label: this.$t('bulkDeliveryRecord.drcg'),
          },
        ], // 订单状态
        inputInfo: {
          fileName: null,
    ...
    methods: {
      async deliverList() {
        const res = await deliverList({
          api: "admin.user.uploadRecordList",
      ...
      Delet(value) {
        this.$confirm(this.$t('bulkDeliveryRecord.scts'), this.$t('bulkDeliveryRecord.ts'), {
          ...
          .then(() => {
            delDelivery({
              api: "admin.user.delUploadRecord",
              id: value.id,
            }).then((res) => {
              if (res.data.code == "200") {
                this.$message({
                  type: "success",
                  message: this.$t('bulkDeliveryRecord.sccg2'),
                  offset: 100,
                });
