<template>
  ...
  <div class="basic-configuration merchants-list"  >
      ...
      <el-form
        ...
        class="demo-ruleForm"
      >
        <el-form-item :label="$t('systemConfig.dly')" prop="logonLogo" required>
          <l-upload
            :limit="1"
            :size="152"
            :heightSize="50"
...
export default {
  ...
  methods: {
    ...
    // 设置基础配置
    submitForm( ) {
        if( !this.ruleForm.h_Address){
           this.$message({
              message: this.$t('systemManagement.qsryddtishi'),
              type: 'error',
              offset: 102
            })
          return
        }
         if( !this.ruleForm.watermarkName){
           this.$message({
              message: this.$t('systemManagement.qtxmc'),
              type: 'error',
              offset: 102
