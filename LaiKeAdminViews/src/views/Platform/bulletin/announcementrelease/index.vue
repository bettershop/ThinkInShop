      ...
      <div class="Commencement-date">
        <el-form-item :label="$t('bulletin.announcementrelease.jsfxz')" prop="storeTypes">
          <el-checkbox-group v-model="ruleForm.storeTypes">
            <el-checkbox
              v-for="(item, index) in storeTypeOptions"
              :key="index"
              :label="item.code"
...
export default {
  ...
  data() {
    return {
      ...
      storeTypeOptions: [
        { code: 0, label: this.$t('bulletin.announcementrelease.SelectOptionList[0]') }, // 对应0
        { code: 1, label: this.$t('bulletin.announcementrelease.SelectOptionList[1]') },     // 对应1
        { code: 2, label: this.$t('bulletin.announcementrelease.SelectOptionList[2]') },     // 对应2
        { code: 3, label: this.$t('bulletin.announcementrelease.SelectOptionList[3]') },   // 对应3
        { code: 4, label: this.$t('bulletin.announcementrelease.SelectOptionList[4]') }      // 对应4
      ],
      storeTypes: [],
      ruleForm: {
        goodsTitle: '',
      ...
      this.$refs[formName].validate(async (valid) => {
        ...
        if (valid) {
          try {
            ...
            const storeTypesStr = this.ruleForm.storeTypes
              .filter(code => typeof code === 'number') // 确保是数字
              .join(','); // 核心：数组转逗号分隔字符串
