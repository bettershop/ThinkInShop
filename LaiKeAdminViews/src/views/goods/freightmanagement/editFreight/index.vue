    ...
    >
      <div class="notice">
        <el-form-item :label="$t('addtemplate.ssgj')" class="title calculation" prop="country_num">
          <el-select   filterable v-model="ruleForm.country_num" :placeholder="$t('addtemplate.qxzssgj')">
        ...
        <el-form-item :label="$t('addtemplate.yfyz')" class="title calculation" prop="lang_code">
          <el-select disabled  filterable v-model="ruleForm.lang_code" :placeholder="$t('addtemplate.qxzyz')">
        ...
        <el-form-item :label="$t('addtemplate.mryf')" prop="defaultFreight">
          <div class="model_price">
            ...
            <div style="margin-left: 6px; margin-bottom: 10px">
              ...
              >
                <template slot="append">{{ $t('addtemplate.yuan') }}</template>
              </el-input>
            </div>
          </div>
        </el-form-item>
              ...
              <el-table-column :label="$t('addtemplate.sdd')" width="200">
                <template slot-scope="scope">
                  <div>
                    ...
                    <!-- <div v-if="disabled == false" class="edit_one" @click="edit(scope.row,scope.$index)">|&nbsp;编辑</div>
										<div v-if="disabled" class="edit_one"	@click="dialogShow2(scope.row,scope.$index)">|&nbsp;查看</div> -->
                    <div
                      class="edit_one"
        ...
        <el-form-item class="rule-block" v-if="isCN" :label="$t('addtemplate.bpsdq')">
          <div>
            <el-button
              v-if="disabled == false"
              @click="setRegion()"
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rule: {
        country_num: [
          { required: true, message: this.$t('addtemplate.qxzssgj') , trigger: 'blur' }
        ],
        lang_code: [
          { required: true, message: this.$t('addtemplate.qxzyz') , trigger: 'blur' }
        ],
        name: [
          {
            required: true,
  ...
  async created () {
    ...
    let newVal = this.$route.query.lang_code
    this.isCN  = false;
  ...
  watch: {
    ...
    'ruleForm.type': {
      handler: function (newVal, oldVal) {
        if (this.$route.query.name != 'view' && oldVal != '') {
          this.ruleForm.num1 = ''
          this.ruleForm.num2 = ''
          this.ruleForm.num3 = ''
          this.ruleForm.num4 = ''
  ...
  methods: {
     ...
     * @param threeIdsList 存放三级id 的数组 、 存在二三级名称的数组
     * @param normalTetx 根据什么 关键字 查找; 默认 id 地区id
     */
    ...
    // 		if (province.district_name === this.ruleForm.province) {
    // 			await this.getShi(province.id, true);
    // 			for (const city of this.shiList) {
    // 				if (city.district_name === this.ruleForm.city) {
    // 					await this.getXian(city.id, true);
    // 					break;
    // 				}
    // 			}
    // 			break;
    ...
    edit (row, index) {
      ...
      //   this.cascader = JSON.parse(row.canshuID)
      this.cascader3 = JSON.parse(row.canshuID)
      this.sub = index
    },
    //移除table行
    Delete (index, row) {
