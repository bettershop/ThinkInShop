  ...
  <div class="container">
    <div class="btn-nav">
      <!-- <el-radio-group fill="#2890ff" text-color="#fff" v-model="radio1">
        <el-radio-button :label="$t('physicalgoods.swsp')" @click.native.prevent="$router.push('/goods/goodslist/physicalgoods')"></el-radio-button>
        <el-radio-button :label="$t('physicalgoods.xnsp')" @click.native.prevent="$router.push('/goods/goodslist/virtualgoods')"></el-radio-button>
      </el-radio-group> -->
      <l-tab
        ref="navLabel"
    ...
    <div class="jump-list">
      ...
      <el-button
        v-has-permi="'/goods/goodslist/batchImport'"
        class="batchImport"
        type="primary"
        icon="el-icon-upload2"
        @click="$router.push('/goods/goodslist/batchImport')"
        >{{ $t('physicalgoods.pldr') }}</el-button
      >
      <el-button
        v-has-permi="'/goods/goodslist/bulkDeliveryRecord'"
        class="batchImport"
        type="primary"
        icon="el-icon-document"
        @click="$router.push('/goods/goodslist/bulkDeliveryRecord')"
      ...
      >
      <el-button
        v-has-permi="'butSetFreight'"
        class="  laiketui laiketui-shangjia"
        type="primary"
        v-if="idList.length>0"
        @click="butEventer($t('physicalgoods.szyf'))"
        >{{ $t('physicalgoods.szyf') }}</el-button
      >
      <el-button
        v-has-permi="'bugEarlyWarningSetting'"
        class="  laiketui laiketui-shangjia"
        type="primary"
        v-if="idList.length>0"
        @click="butEventer($t('physicalgoods.yjsz'))"
        >{{ $t('physicalgoods.yjsz') }}</el-button
      >
      <el-button
      ...
      <div class="show-goods">
        <div class="center_a">
          <span>{{ $t('physicalgoods.sfxsysqsp') }}</span>
          <el-switch
            v-model="value2"
            :active-value="1"
            :inactive-value="0"
    ...
    <div class="menu-list" ref="tableFather">
      ...
      >
        ...
        <el-table-column prop="price" :label="$t('physicalgoods.jg')">
          <template slot-scope="scope">
          {{laikeCurrencySymbol}}{{scope.row.price}}
          </template>
        </el-table-column>
        ...
        <el-table-column
          prop="name"
          :label="$t('physicalgoods.ssdp')"
          width="200"
        >
        </el-table-column>
        <el-table-column prop="volume">
          <template #header>
              <p @click="clickSort" class="sales-volume">
                  {{$t('physicalgoods.xl')}}
      ...
      >
        <div class="body-box">
          ...
          <el-form ref="ruleForm" :model="form" label-width="100px">
            <!-- 设置显示位置 -->
            <el-form-item :label="title" v-if="title == $t('physicalgoods.xzwz')">
            ...
            <!-- 设置运费 -->
            <el-form-item :label="title" v-if="title == $t('physicalgoods.szyf')">
              <el-select
                class="select-input"
             ...
             <!-- 库存预警 -->
             <el-form-item v-if="title == $t('physicalgoods.yjsz')" >
                <div class="kcyj">
                    <span>{{ $t('physicalgoods.spmygg1') }}</span>
                    <el-input
                    v-model="form.forewarning"
                    @input="form.forewarning = oninput2(form.forewarning)">
                    </el-input>
                    <span>{{ $t('physicalgoods.spmygg2') }}</span>
                </div>
            </el-form-item>
          </el-form>
...
export default {
  ...
  data () {
    return {
      laikeCurrencySymbol:'￥',
      radio1: this.$t('physicalgoods.swsp'),
      classList: [],
      brandList: [],
      activityList: [],
      statusList: [
      ...
      labelList: [],
      classnotset:null,//为1的时候 sql改成查询为空的分类
      brandnotset:null,//为1的时候 sql改成查询为空的品牌
      inputInfo: {
        goodsClass: '',
        brand: '',
        type: '',
  ...
  created () {
    ...
    this.choiceClasss().then(() => {
      if (this.$route.params && this.$route.params.inputInfo &&
    ...
    if ( this.$route.params && this.$route.params.pageSize) {
  ...
  methods: {
    ...
    async getFreightInfos() {
      const goodsIds = this.idList.map(v=>v.id).join(',')
      const res = await getFreightInfo({
        api: "admin.goods.BatchObtainShippingFees",
        goodsIds
      });
    ...
    submitForm(){
      let data = {}
      if(this.title ==this.$t('physicalgoods.yjsz')){
        // 库存预警
        if(this.form.forewarning == ''){
          this.warnMsg(this.$t('releasephysical.qsrkcyj'))
          return
        }
        data.api = 'admin.goods.BatchWarning'
        data.min_inventory   = this.form.forewarning
      }else if(this.title == this.$t('physicalgoods.szyf')){
        // 设置运费
    ...
    butEventer(type){
      ...
      // 不同店铺不能批量设置运费
      if(type == this.$t('physicalgoods.szyf')){
        const mchList =  Array.from(new Set(this.idList.map(v=>v.mch_id)))
        console.log(mchList)

        if(mchList && mchList.length>1){
        this.warnMsg(this.$t('physicalgoods.freightStop'))
          return
        }
    ...
    async indexs () {
      ...
      const res = await index({
        ...
        IsItDescendingOrder:this.IsItDescendingOrder //销量排序
      })
    ...
    Copy (value,item) {
      ...
      console.log("=========");
      console.log(value);
      value.org_lang_code = value.lang_code;
      ...
      this.$router.push({
        ...
        query: {
          ...
          org_lang_code: value.org_lang_code,
          dictionaryNum: this.dictionaryNum,
          pageSize: this.pageSize,
          classId: value.product_class,
          inputInfo: this.inputInfo
