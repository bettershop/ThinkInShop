            ...
            <div>   
                <ul class='evaluate-classify'>
                    ...
                    <li class='evaluate-classify_li' @tap="_comment(4)" :class='allevaluat5==4?"active":""'>
                        <div>
                            <span>{{language.evaluate.evaluate.dai_img}} </span>
                        </div>
                    </li>
                </ul>
            </div>
                ...
                <li v-for='(item,index) in isData' :key='index' >
                    ...
                    <div class="center">
                        ...
                        <div class='content_img'>
                            <image class='img' v-for='(url,imgindex) in item.commentImgList' :key='imgindex' @click="_bigimg(url)" :src='url' mode="aspectFit"/>
                        </div>
                    </div>
                    <!-- 追评 -->
            ...
            <view @touchmove.stop.prevent class="bounced" v-if="mask">
                <view class="bounced-box">
                    <view class="bounced-box-box">
                        ...
                        <image :src="guanbi" class="bounced-box-box-img" @click="maskclick(item.commentsId)"></image>
                    </view>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            nato(item){
                uni.navigateTo({
                    url:'/pagesC/evaluate/Evaluation/evaluation_subsidiary?id=' + item.id + '&item=' + encodeURIComponent(JSON.stringify(item))
                })
            },
            //放大图片
            _bigimg(e) {
            ...
            _axiosPid(){
                let data = {
                    api:'plugin.integral.AppIntegral.getGoodsInfoById',
                    goodsId: this.pid,
                }
                this.$req.post({data}).then(async res => {
                    if(res.code ==200){
            ...
            _huifu(){
                this.mask = !this.mask
                let data = {
                    api: 'plugin.integral.AppIntegral.replyComments',
