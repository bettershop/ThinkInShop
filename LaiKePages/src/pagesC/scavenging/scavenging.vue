<template>
    <div>
        <heads :title='language.scavenging.title'></heads>
        <!--  没有搜索商品显示   -->
        <div class='empty' v-if='!goods.length&&!load'>
            <img class='empty_img' :src="empic_img"/>
            <p class='empty_p'>{{language.scavenging.search_tips}}</p>
            <div class='empty_input'>
                <span>{{language.scavenging.Commodity}}</span>
                <span>{{result}}</span>
            </div>
            <div class='empty_search'>
                <input type="number" :placeholder="language.scavenging.Commodity_placeholder" @focus="_focus"/>
                <img :src="search_img"/>
            </div>
        </div>
        <!--  有商品显示    -->
        <ul v-if='goods.length&&!load'>
            <li class='sacven_goods' v-for='(item,index) in goods' @click="_goods(item.id)" :key='index'>
                ...
                <div class='sacven_right'>
                    ...
                    <p class='sacven_size'>{{language.scavenging.Specifications}}：{{item.subtitle}}</p>
                    <p class='sacven_price'>{{LaiKeTuiCommon.DEFAULT_STORE_SYMBOL}}{{Number(item.price)}}</p>
                </div>
            </li>
        </ul>
        <div class='load' v-if='load'>
            <div>
                <img :src="load_img"/>
                <p>{{language.scavenging.Loading}}</p>
            </div>
        </div>
    </div>
</template>
...
<style scoped lang="less">
    @import url("../../static/css/scavenging/scavenging.less");
</style>
