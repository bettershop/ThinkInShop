<template>
  <div class="login-button">
	...
	<text @tap.shop="navTo('/pagesD/login/retrievePassword/authentication')">{{ language.newlogin.wjmm }}</text> 
    </div>
    <div class="login-agreement">
      ...
      <button :class="[isReading ? '' : 'uni-btn-disabled','uni-btn']" :disabled="!isReading" @click="handleLogin">{{ language.newlogin.dl }}</button>
    <div class="login-register">
      <span @tap.shop="navTo('/pagesD/login/captchaLogin/index')">{{ language.newlogin.yzmdl }}</span>
      <span class="login-register-text">|</span>
      <span @tap.shop="navTo('/pagesD/login/registration/index')">{{ language.newlogin.ljzc }}</span>
    </div>
  </div>
</template>
