<template>
	<view class="swiperBg" :style="{marginTop: marginTop + 'px', background: bgColor2[0].item}" >
		<!-- 单图模式 -->
		<block>
			<view class="colorBg" :style="'background: linear-gradient(90deg, '+ bgColor[0].item +' 50%, '+ bgColor[1].item +' 100%);'" v-if="swiperType == 1"></view>
			<view class="swiper" :class="[imgConfig?'':'fillet']"  :style="'padding: 0 '+ paddinglr +'px;'"
			 v-if="imgUrls.length">
                
				<swiper class="swiper-content" :class="[imgConfig?'':'fillet']" :style="'height:'+ (imageH) +'px!important;'" :autoplay="true"
					...
					<block v-for="(item,index) in imgUrls" :key="index">
						<swiper-item>
							<view @click="$emit('toUrl', item.info[1].value)" class='slide-navigator acea-row row-between-wrapper'>
								<image :style="{height: (imageH) +'px'}" :src="item.img" class="slide-image"></image>
							</view>
						</swiper-item>
					</block>
				</swiper>
