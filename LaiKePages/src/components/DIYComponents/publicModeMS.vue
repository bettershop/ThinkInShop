<template>
    <view class="isPublicModeMS" :style="{background: `linear-gradient(90deg,${bgColor2[0].item} 0%,${bgColor2[1].item} 100%)`,marginTop:`${pxToRpxNum(mTOP)}rpx`}" >
        ...
        <template v-if="secList && secList.length">
            <view class="content_newFive" >
                <view>
                    ...
                    <view class="content_newFive_tilte2 more" @tap="_seeMore('/pagesB/seckill/seckill')">
                        {{language.home.more}}
...
<script>
    export default{
        ...
        created() {
            ...
            console.log(this.dataConfig,'dataConfigdataConfig')
            if( this.dataConfig && Object.keys( this.dataConfig).length>0)
            { 
                this.mTOP = this.dataConfig.mbConfig.val;
                        
                this.bgColor2 = this.dataConfig.bgColor.color;
            }
            this.getSeckill()
        },
        watch: {
            dataConfig:{
                handler(newVal,lodVla){
                    ...
                    if(newVal && Object.keys(newVal).length>0){ 
                        this.mTOP = newVal.mbConfig.val; 
                        this.bgColor2 = newVal.bgColor.color;
                    }
                },
                deep:true,  
            }
        ...
        methods:{
            setDefaultImage(index){
                if (this.secList && this.secList[index]) {
