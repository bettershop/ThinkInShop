    ...
    <view class="mpvue-picker">
        ...
        <view class="mpvue-picker-content " :class="{ 'mpvue-picker-view-show': showPicker }">
            <view class="mpvue-picker__hd" catchtouchmove="true">
                <view class="mpvue-picker__action" @click="pickerCancel">{{language.mpvuePicker.cancel}}</view>
                <view class="mpvue-picker__action" :style="{ color: themeColor }" @click="pickerConfirm">{{language.mpvuePicker.confirm}}</view>
            </view>
            <!-- 单列 -->
            <picker-view
            ...
            >
                <block>
                    <picker-view-column>
                        <view class="picker-item" v-for="(item, index) in pickerValueSingleArray" :key="index">{{ item }}</view>
                    </picker-view-column>
                </block>
            </picker-view>
            <!-- 时间选择器 -->
    ...
    methods: {
        initPicker(valueArray) {
            ...
            if (this.mode === 'selector') {
                ...
                for (let i = 0; i < 24; i++) {
                    hourArray.push({
                        value: i,
                        label: i > 9 ? `${i} ${this.language.mpvuePicker.hour}` : `0${i} ${this.language.mpvuePicker.hour}`
                    });
                }
                for (let i = 0; i < 60; i++) {
                    minuteArray.push({
                        value: i,
                        label: i > 9 ? `${i} ${this.language.mpvuePicker.min}` : `0${i} ${this.language.mpvuePicker.min}`
                    });
                }
        ...
        pickerChange1(e) {
            ...
            let pickObj = {
                ...
                label: this._getPickerLabelAndValue(this.pickerValue, this.mode)
            };
            this.$emit('onChange', pickObj);
        },
        pickerChangeMul(e) {
        ...
        _getPickerLabelAndValue(value, mode) {
            ...
            // selector
            if (mode === 'selector') {
                pickerLable = this.pickerValueSingleArray[value];
