<template>
    <div class="mpvue-picker">
        ...
        <div v-if="isType == 0" class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
            <div class="mpvue-picker__hd head" catchtouchmove="true">
                <div class="mpvue-picker__action cancel" @click="pickerCancel">
                    {{language.delModel.cancel}}
                </div>
                <div class="mpvue-picker__action confirm" :style="{color:themeColor}" @click="pickerConfirm">
                    {{language.myStore.addVerification.qd}}
                </div>
            </div>
        ...
        <div v-else class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
    ...
    export default {
        data () {
            return {
                ...
                showPicker: false,
                /* 选择时间段 */
                time: [hours[0] + ':' + minutes[0], hours[0] + ':' + minutes[0]]
            }
        },
        props: ['themeColor', 'urseTime','isType'],
        methods: {
            show (data) {
                if (data) {
                    let sparr = []
                    ...
                    if(sparr[0]){
                        sparr[0].filter((item,index)=>{
                            sparr[0][index] = Number(item)
                        })
                    }
