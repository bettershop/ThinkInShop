<template>
    ...
    <!-- 选择配送方式 -->
        <template>
            <view class="psfs">
                <view>{{ language.chooseAddress.receivingWay }}</view>
    ...
    <!-- 配送方式：商家配送 -->
        <template v-if='is_express && zpStatus && is_choose_type == 2'>
            <view class="sjps">
                <view>{{ language.chooseAddress.deliveryTimeOptional }}</view>
                <view class="sjps-time">
                    <!-- 未选择配送时间 -->
                    <view class="sjps-time-no" v-if="!virtualTimeEnd" @tap="virtualTime = true">
                        <span>{{ language.chooseAddress.pleaseSelectDeliveryTime }}</span><image :src="jiantou" mode="widthFix"></image>
                    </view>
                    <!-- 已选择配送时间 -->
    ...
    <!-- 商家配送 选择配送时间弹窗 -->
        <view class="virtual-time" v-if="virtualTime" @tap="_virtualTimeEnd" @touchmove.stop.prevent>
            <view @tap.stop class="safe-area-inset-bottom">
                <view>
                    {{ language.chooseAddress.selectDeliveryTime }}
...
<script>
    ...
    export default {
        ...
        created() {
            this.is_choose_obj.title = this.language.chooseAddress.receivingWay
            this.buildPsList()
            this.setLang();
        },
        mounted() {
        ...
        watch: {
            ...
            is_express: {
                handler(val) {
                    this.syncChooseIndex(this.normalizeType(val))
                },
                immediate: true
            }
        },
        ...
        methods: {
            ...
            syncChooseIndex(type) {
                let index = this.isPsTypeList.findIndex(item => item === type)
                if (index < 0) {
                    index = 0
                    type = this.isPsTypeList[0] || 1
                }
                this.is_choose_index = index
                this.is_choose_type = type
            },
            buildPsList() {
                const list = [this.language.chooseAddress.kdps]
                const typeList = [1]
                if (this.shop_status && this.is_supplier_pro) {
                    list.push(this.language.chooseAddress.smzt)
                    typeList.push(0)
                }
                if (this.zpStatus) {
                    list.push(this.language.chooseAddress.sjps)
                    typeList.push(2)
                }
                this.isPsList = list
                this.isPsTypeList = typeList
                this.syncChooseIndex(this.normalizeType(this.is_express))
            },
            //商家配送时间（当前至往后7天时间列表）
            getTime(index){
            ...
            _virtualTimeEnd(type){
                ...
                if(type == 'quxiao'){
                    ...
                    if(deliveryTime && deliveryPeriod){
                        this.virtualTimeEnd = deliveryTime + '  ' + (deliveryPeriod == 1 ? this.language.chooseAddress.morning : this.language.chooseAddress.afternoon)
                    }
                }
            },
            //显示
            ...
            _choose(index){
                ...
                const type = this.isPsTypeList[index] !== undefined ? this.isPsTypeList[index] : 1
                this.is_choose_type = type
