<template>
    <view class="skeleton-p" v-if="loading" :style="{
        ...
        <view v-for="item in RectNodes" :key="guid()" :class="[animation ? 'skeleton-fade' : '']" :style="{
			width: item.width + 'px', 
        ...
        <view v-for="item in circleNodes" :key="guid()" :class="animation ? 'skeleton-fade' : ''" :style="{
			width: item.width + 'px', 
			height: item.height + 'px', 
			backgroundColor: elColor, 
        ...
        <view v-for="item in filletNodes" :key="guid()" :class="animation ? 'skeleton-fade' : ''" :style="{
			width: item.width + 'px', 
			height: item.height + 'px', 
			backgroundColor: elColor, 
...
<script>
     ...
     * @property {String || Number} border-radius skeleton-fillet类名元素，对应的骨架块的圆角大小，单位rpx（默认10）
     * @property {Boolean} loading 是否显示骨架组件，请求完成后，将此值设置为false（默认true）
     * @example <skeleton :loading="true" :animation="true"></skeleton>
     */
    export default {
        name: "skeleton",
        props: {
            ...
            // 圆角值，只对类名为skeleton-fillet的元素生效，为数值，不带单位
            borderRadius: {
                type: [String, Number],
                default: "10"
            },
